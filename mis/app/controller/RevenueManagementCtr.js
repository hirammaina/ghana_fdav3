...
Ext.define("Admin.controller.RevenueManagementCtr", {
  ...
  listen: {
    controller: {
      "*": {
        ...
        funcPrintRetentionPaymentsStatement:
          "funcPrintRetentionPaymentsStatement",
      },
    },
  },
  saveApplicationInvoicingDetails: function (btn) {
    var me = this,
      ...
      application_id = invoice_pnl
        .down("hiddenfield[name=application_id]")
        .getValue(),
      application_code = invoice_pnl
      ...
      paying_currency_id = invoice_pnl
    ...
    Ext.MessageBox.confirm(
      "Confirm",
      "Are you sure to save invoice details?",
      function (button) {
        if (button === "yes") {
  ...
  commitApplicationInvoicingDetails: function (btn) {
    ...
    var me = this,
      ...
      application_feetype_id = invoice_pnl
  ...
  funcCancelGeneratedInvoice: function (btn) {
    ...
    Ext.MessageBox.confirm(
      ...
      "Do you want to cancel the generate Proforma Invoice?",
      function (button) {
        if (button === "yes") {
  ...
  funLinkBatchInvoiceDetails: function (btn) {
    ...
    Ext.each(selected_records, function (item) {
      ...
      batch_invoicestore.remove(
        batch_invoicestore.findRecord(
          "invoice_id",
          record_id,
          0,
          false,
  ...
  onAdhocViewApplicationDetails: function (record) {
    ...
    if (!tab) {
      ...
      if (newTab.down("hiddenfield[name=permitapplication_code]")) {
        newTab
          .down("hiddenfield[name=permitapplication_code]")
          .setValue(permitapplication_code);
      }
  ...
  prepareRevewnuCreditNoteREquestDetails: function () {
    ...
    var me = this,
      ...
      sub_module_id = activeTab
        ...
        .getValue(),
      cancellation_id = activeTab
        .down("hiddenfield[name=cancellation_id]")
        .getValue();
    var action_url = "revenuemanagement/prepareCancellationREquestDetails";
    ...
    Ext.Ajax.request({
      ...
      success: function (response) {
        ...
        if (success == true || success === true) {
          ...
          invoice_cancellationfrm
            .down("numberfield[name=total_amount]")
            .setValue(results.credit_note_amount * results.exchange_rate);
  ...
  prepareAdhocInvoiceRequestpnl: function () {
    ...
    var me = this,
      ...
      adhocinvoiceotherdetailsFrm = activeTab.down(
        "adhocinvoiceotherdetailsFrm"
      ),
      application_id = activeTab
        .down("hiddenfield[name=active_application_id]")
        .getValue(),
  ...
  prepareInspectionatownerpremreceivingpnl: function () {
    ...
    if (!invoiceSummaryGrid) {
      invoiceSummaryGrid = activeTab.down("paymentinvoicingcostdetailsgrid");
    }
    console.log(invoiceSummaryGrid);
    var invoiceSummaryStore = invoiceSummaryGrid.store;
  ...
  showInvoiceCancellationReqSubmissionWin: function (btn) {
    ...
    var mainTabPanel = this.getMainTabPanel(),
      ...
      valid = form.isValid(),
      reason_for_cancellation = activeTab
        .down("form")
        .down("textarea[name=reason_for_cancellation]")
        .getValue();
    ...
    if (valid == true || valid === true) {
      showWorkflowSubmissionRevenueWin(
        application_code,
        workflow_stage_id,
        "revenuerequestsubmissionsfrm",
        winWidth,
        extraParams
      );
    } else {
  ...
  showCreditNoteRequestReqSubmissionWin: function (btn) {
    ...
    var mainTabPanel = this.getMainTabPanel(),
      ...
      workflow_stage_id = activeTab
        ...
        .getValue(),
      receipt_id = activeTab.down("hiddenfield[name=receipt_id]").getValue(),
      exchange_rate = activeTab
        .down("hiddenfield[name=exchange_rate]")
        .getValue(),
      credit_note_amount = activeTab
        .down("textfield[name=credit_note_amount]")
        .getValue(),
      valid = form.isValid(),
      reason_for_request = activeTab
        .down("form")
        .down("textarea[name=reason_for_request]")
        .getValue();
    currency_id = "";
  ...
  onPaymentProcessRequest: function (sub_module_id, dashwrapper) {
    ...
    var me = this,
      ...
      dashboardWrapper = activeTab.down(dashwrapper),
      module_id = activeTab.down("hiddenfield[name=module_id]").getValue();
    workflow_details = getInitialWorkflowDetailsNoProcess(
      module_id,
      sub_module_id
    );
  ...
  getRetentionPaySelectedTradersDetails: function () {
    var me = this,
      mainTabPanel = me.getMainTabPanel(),
      activeTab = mainTabPanel.getActiveTab(),
      applicantselection = activeTab.down(
        "grid[name=retentionapplicantsselectiongrd]"
      );
    (store = applicantselection.store), (applicant_id = "");
  ...
  refreshLinkretentionchargesgrid: function (me) {
    var store = me.getStore(),
      grid = me.up("grid"),
      retention_yearfrom = grid
        .down("combo[name=retention_yearfrom]")
        .getValue(),
  ...
  refreshretentionchargesinvoicesgrid: function (me) {
    var store = me.getStore(),
      ...
      retention_yearto = grid.down("combo[name=retention_yearto]").getValue();
    section_id = grid.down("combo[name=section_id]").getValue();
    retention_status_id = grid
      .down("combo[name=retention_status_id]")
      .getValue();
  ...
  funcPrintRetentionPaymentsStatement: function (btn) {
    ...
    redirect =
      "reports/printRetentionPaymentsStatement?retention_yearfrom=" +
      retention_yearfrom +
      "&retention_yearto=" +
      retention_yearto +
      "&trans_datefrom=" +
  ...
  onRetentionApplicantAddSelectedList: function (btn) {
    var pnl = this.getRetentionchargespnl(),
      ...
      retentionapplicantsselectiongrd = pnl.down(
        "grid[name=retentionapplicantsselectiongrd]"
      ),
  ...
  onRetentionPaymentApplicantSelectionListDblClick: function (
    view,
    record,
    rr
  ) {
  ...
  revenueimportexportspermitgridDblClick: function (view, record) {
    ...
    activeTab
      .down("displayfield[name=impexport_permitno]")
    ...
    activeTab
      .down("hiddenfield[name=applicant_id]")
      .setValue(record.get("applicant_id"));

    if (Ext.getStore("premiinspimpextpermitsproductsstr")) {
      Ext.getStore("premiinspimpextpermitsproductsstr").load({
        param: { application_code: record.get("application_code") },
      });
    }
    win.close();
  },
  refreshpremiinspimpextpermitsproductsgrid: function (me) {
    var store = me.getStore(),
      ...
      mainTabPanel = me.getMainTabPanel(),
      activeTab = mainTabPanel.getActiveTab(),
      permitapplication_code = activeTab
        .down("hiddenfield[name=permitapplication_code]")
        .getValue();
