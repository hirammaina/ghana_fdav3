...
'controlleddrugspermitsreceivingwizard button[name=generate_invoice]': {
    ...
    listen: {
        controller: {
            '*': {// This selector matches any originating Controller, you can specify controller name instead of *
                ...
                showPromotionalApplicationChecklists:'showPromotionalApplicationChecklists',
                ...
                funGroupedConfirmUploadedPaymentsDetails:'funGroupedConfirmUploadedPaymentsDetails',
    ...
    funcAddVariationCOnfigApplicationParamter:function(btn){
        ...
        funcShowCustomizableWindow('Add New Parameter', '55%', childXtype, 'customizablewindow');
    },
...
showApplicationEvaluationUploads:function(btn){
        ...
        tc_reviewform.down('combo[name=decision_id]').setValue(record.get('tc_decision_id'));
        ...
        if(tc_reviewform.down('textarea[name=certificate_expiry_statement]')){
            ...
            tc_reviewform.down('textarea[name=certificate_expiry_statement]').setValue(record.get('certificate_expiry_statement'));

        }
        if(tc_reviewform.down('datefield[name=certificate_expiry_date]')){

            tc_reviewform.down('datefield[name=certificate_expiry_date]').setValue(record.get('certificate_expiry_date'));
    ...
    funcUploadTCMeetingtechnicalDocuments:function(btn){
        ...
        if(containerPnl.down('premiseMeetingDetailsFrm')){
            ...
            meeting_id = containerPnl.down('premiseMeetingDetailsFrm').down('hiddenfield[name=id]').getValue();
        }
        else{
            if(containerPnl.down('premisesinspectiondetailsfrm')){
                ...
                meeting_id = containerPnl.down('inspectionbasicdetailsfrm').down('hiddenfield[name=id]').getValue();
    ...
    refreshApplicationDocUploadsGrid: function (me) {
        var store = me.store,
            ...
            mainTabPanel = this.getMainTabPanel(),
            activeTab = mainTabPanel.getActiveTab(),   
            business_type_id,prodclass_category_id;
            ...
            if(application_code >0){
                ...
                if(module_id == 2){
                        if(activeTab.down('hiddenfield[name=business_type_id]')){
                            business_type_id = activeTab.down('hiddenfield[name=business_type_id]').getValue();
                        }
                        else if(grid.up('window')){

                            business_type_id = grid.up('window').down('hiddenfield[name=business_type_id]').getValue();
    ...
    addApplicationIdCodeParams: function (me) {
        ...
        if(me.up('window')){
            ...
            if(activeTab.down('hiddenfield[name=group_application_code]')){
                ...
                group_application_code = activeTab.down('hiddenfield[name=group_application_code]').getValue();
            }
        ...
        if(me.up('window')){
            ...
            var popupview = payment_pnl
                ...
                if(popupview.down('hiddenfield[name=group_application_code]')){
                    group_application_code = popupview.down('hiddenfield[name=group_application_code]').getValue();
                }
                else{
                    application_id = 0;
                    application_code =0
    ...
    onViewApplicationDetails: function (record) {
        ...
        var me = this,
            ...
            has_assessmentassignment_check =record.get('has_assessmentassignment_check'),
...
if(is_receipting_stage == 1){
  //  workflow_details = getAllWorkflowDetails(process_id, workflow_stage_id);
    ...
    me.funHelperViewApplicationDetails(is_readonly,workflow_stage,ref_no,process_id,workflow_stage_id,tracking_no,view_id,isGeneral,record,workflow_details);
}
        ...
        if(usr_to >0){
            if(usr_to != user_id){
                Ext.Msg.show({
                    ...
                    msg : 'Are you sure you want to continue with reassignment from '+to_user+' of the selected Application(No for Readonly)?',
                    width : 600,
                    closable : false,
    ...
    funHelperViewApplicationDetails:function(is_readonly,workflow_stage,ref_no,process_id,workflow_stage_id,tracking_no,view_id,isGeneral,record,workflow_details){
        var me = this,
        mainTabPanel = me.getMainTabPanel();
        /*
        workflow_details = getAllWorkflowDetails(process_id, workflow_stage_id);
    ...
    prepareApplicationBaseDetails: function (tab, record) {
        ...
        if(tab.down('hiddenfield[name=group_application_code]')){
            ...
            tab.down('hiddenfield[name=group_application_code]').setValue(group_application_code);
        }
    },
    ...
    showApplicationCommentsGeneric: function (item, application_id, application_code) {
        ...
        child.setHeight(450);
        funcShowCustomizableWindow('Workflow/Tasks Comments & Recommendation', '50%', child, 'customizablewindow');
    },
