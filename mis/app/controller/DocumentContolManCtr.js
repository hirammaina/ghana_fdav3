Ext.define("Admin.controller.DocumentContolManCtr", {
  ...
  listen: {
    controller: {
      "*": {
        showControlDocumentProcessWorkflow:
          "showControlDocumentProcessWorkflow",
  ...
  showControlDocumetnReceivinSubmissionWin: function (btn) {
    ...
    if (valid) {
      Ext.Ajax.request({
        ...
        success: function (response) {
          ...
          if (!success) {
            ...
            toastr.warning(
              "Please Upload the required Control Document Details!!",
              "Warning Response"
            );
            return false;
          } else {
      ...
      Ext.getBody().unmask();
      toastr.warning(
        "Please Enter All the required Document Details!!",
        "Warning Response"
      );
      return;
    }
  ...
  validateNewControlDocumentSubmission: function (application_code) {
    var mainTabPanel = this.getMainTabPanel(),
      activeTab = mainTabPanel.getActiveTab(),
      newdocumentcontrolrequestfrm = activeTab.down(
        "#documentcontrolrequestfrm"
      );

    if (!newdocumentcontrolrequestfrm.isValid()) {
      toastr.warning(
        "Please Enter All the required Control Document Details!!",
        "Warning Response"
      );
      return false;
    }
  ...
  prepareNewControlDocumentRequest: function () {
    ...
    activeTab
      .down("#documentcontrolrequestfrm")
      .getViewModel()
      .set("isReadOnly", is_readonly);
  ...
  refreshpreviouscontrollleddocumentsaccessgrid: function (me) {
    var store = me.store,
      grid = me.up("grid"),
      reg_doccontrolreview_id = grid
        .down("hiddenfield[name=reg_doccontrolreview_id]")
        .getValue();
    alert();
    store.getProxy().extraParams = {
  ...
  funcCOntrolDocumentOnHome: function (btn) {
    var me = this,
      ...
      dashboardWrapper = activeTab.down(
        "#controldocument_managementdashwrapper"
      );
