...
Ext.define("Admin.controller.WorkflowManagementCtr", {
  ...
  showPaymentsApplicationSubmissionWin: function (btn) {
    ...
    if (valid == true || valid === true) {
      ...
      if (hasQueries) {
        Ext.getBody().unmask();
        toastr.warning(
          "The Application has open queries, close the queries and proceed.",
          "Query Warning"
        );
        return false;
      } else {
  ...
  showQueryApplicationSubmissionWin: function (btn) {
    ...
    if (!hasQueries) {
      toastr.error(
        "No Query Entered for the selected applications, add to proceed",
        "Failure Response"
      );
    } else {
      storeID = getApplicationStore(module_id, section_id);
  ...
  funcSubmitApplication: function (btn) {
    ...
    if (form.down("hiddenfield[name=non_mainpanel_close]")) {
      non_mainpanel_close = form
        .down("hiddenfield[name=non_mainpanel_close]")
        .getValue();
    }
  ...
  addApplicantDetailstoTab: function (pnl) {
    ...
    //below removed 07.02.2024

    // var application_code = pnl
    //     .down("hiddenfield[name=active_application_code]")
    //     .getValue(),
        ...
        success: function (response) {
          ...
          if (success == true || success === true) {
            txt =
              ...
              " <br> " +
              results.postal_address +
              " <br> " +
              results.telephone_no;
