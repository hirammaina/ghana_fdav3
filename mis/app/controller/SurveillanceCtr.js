...
Ext.define("Admin.controller.SurveillanceCtr", {
  ...
  listen: {
    controller: {
      "*": {
        ...
        pmsApplicationMoreDetails:
          "showSurveillanceApplicationMoreDetailsGeneric",
        pmsApplicationMoreDetailsUnstructured:
          "showSurveillanceApplicationMoreDetailsUnstructured",
      },
    },
  },
  ...
  refreshSamplesGrid: function (me) {
    var store = me.store,
      ...
      application_id = activeTab
        ...
        .getValue(),
      recommendation_id = grid.down("combo[name=decision_id]")
        ? grid.down("combo[name=decision_id]").getValue()
        : "",
  ...
  showPmsPlansAppSelectionList: function (btn) {
    ...
    if (activeTab.down("hiddenfield[name=program_implementation_id]")) {
      var program_id = activeTab
          .down("hiddenfield[name=program_id]")
          .getValue(),
        program_implementation_id = activeTab
          .down("hiddenfield[name=program_implementation_id]")
          .getValue();
    } else {
      var planProgramForm = Ext.ComponentQuery.query(
        ...
        program_id = planProgramForm
          .down("hiddenfield[name=program_id]")
          .getValue(),
        program_implementation_id = planProgramForm
          .down("hiddenfield[name=program_implementation_id]")
          .getValue();
    }
    ...
    childObject
      .down("hiddenfield[name=program_implementation_id]")
      .setValue(program_implementation_id);
  ...
  onPmsApplicationProgramSelectionListDblClick: function (
    view,
    record,
    item,
    index,
    ...
    var regionsStr = planProgramForm.down("combo[name=region_id]").getStore(),
      SampleingSiteStr = planProgramForm
        .down("combo[name=sampling_site_id]")
        .getStore(),
  ...
  onPmsProgramSelectionListDblClick: function (
    view,
    record,
    item,
    index,
  ...
  onPmsPlanSelectionListDblClick: function (
    view,
    record,
    item,
    index,
  ...
  onSampleSelectionListDblClick: function (
    view,
    record,
    item,
    index,
  ...
  onNextCardClickSample: function (btn) {
    ...
    if (nextStep > 0) {
      if (!pms_plan_id) {
        toastr.warning(
          "Please select the PMS Program Plan Implementation Details!!",
          "Warning Response"
        );
        return false;
      }
  ...
  prepareStructuredPmsReceiving: function () {
    ...
    var me = this,
      ...
      dosageFormCol = samplesGrid
        .getColumnManager()
        .getHeaderByDataIndex("dosage_form"),
      productFormCol = samplesGrid
        .getColumnManager()
        .getHeaderByDataIndex("product_form"),
      deviceTypeCol = samplesGrid
        .getColumnManager()
        .getHeaderByDataIndex("device_type"),
      commonNameCol = samplesGrid
        .getColumnManager()
        .getHeaderByDataIndex("common_name"),
      // productsStr = pmsPlanFrm.down('combo[name=product_id]').getStore(),
      regionsStr = pmsPlanFrm.down("combo[name=region_id]").getStore(),
      samplingSiteStr = pmsPlanFrm
        .down("combo[name=sampling_site_id]")
        .getStore(),
      application_id = activeTab
        .down("hiddenfield[name=active_application_id]")
        ...
        success: function (response) {
          ...
          if (success == true || success === true) {
            ...
            if (pmsResults) {
              ...
              regionsStr.load({
                params: { program_id: pmsResults.pms_program_id },
              });

              samplingSiteStr.load({
                params: { program_id: pmsResults.pms_program_id },
              });
            }
  ...
  prepareStructuredPmsTCMeeting: function () {
    ...
    var me = this,
      ...
      application_code = activeTab
        ...
        .getValue(),
      meetingParticipantsGrid = activeTab.down(
        "grid[name=meetingParticipantsGrid]"
      );
    if (application_id) {
      Ext.Ajax.request({
        method: "GET",
  ...
  showSampleCollectionSiteSelectionList: function (btn) {
    ...
    if (activeTab.down("hiddenfield[name=program_implementation_id]")) {
      ...
      if (
        planProgramForm &&
        planProgramForm.down("hiddenfield[name=program_implementation_id]")
      ) {
        program_implementation_id = planProgramForm
          .down("hiddenfield[name=program_implementation_id]")
          .getValue();
    ...
    if (program_implementation_id != "") {
      ...
      toastr.warning(
        "Please select PMS program Implementation Plan!!",
        "Warning Response"
      );
      return false;
    }
  ...
  onCollectionSiteSelectionListDblClick: function (
    view,
    record,
    item,
    index,
  ...
  saveStructuredPmsReceivingBaseDetails: function (btn) {
    ...
    if (collectionSiteFrm.isValid()) {
        ...
        success: function (frm, action) {
          ...
          if (success == true || success === true) {
            if (toaster == 1 || toaster === 1) {
              ...
              collectionSiteForm
                .down("hiddenfield[name=sample_site_id]")
                .setValue(sample_site_id);
              activeTab
      ...
      toastr.warning(
        "Please fill all the required fields(Sample Collection Site)!!",
        "Warning Response"
      );
      return false;
    }
  ...
  saveStructuredPmsEdittingBaseDetails: function (btn) {
    var me = this,
      ...
      sub_module_id = activeTab
        ...
        .getValue(),
      section_id = activeTab.down("hiddenfield[name=section_id]").getValue(),
      planProgramForm = Ext.ComponentQuery.query(
        "#applicationpmsplandetailsfrmRefID"
      )[0],
      wizard = planProgramForm.up("panel"),
      wizard_pnl = wizard.up("panel"),
      ...
      sampling_site_id = wizard_pnl
        .down("combo[name=sampling_site_id]")
        .getValue(),
      program_id = wizard_pnl.down("hiddenfield[name=program_id]").getValue(),
      program_implementation_id = wizard_pnl
        .down("hiddenfield[name=program_implementation_id]")
        .getValue(),
      //'','',,
      zone_id = zone_fld.getValue(),
  ...
  saveSurveillanceSampleDetails: function (btn) {
    ...
    if (tabpanel.down("#pmsprogramplan")) {
      var pmsprogramplan = tabpanel.down("#pmsprogramplan"),
        pms_plan_id = pmsprogramplan
          .down("hiddenfield[name=pms_plan_id]")
          .getValue();
      if (pms_plan_id < 1) {
  ...
  validateStructuredPmsReceivingSubmission: function (btn) {
    ...
    if (!sampleSiteFrm.isValid()) {
      toastr.warning(
        "Please Enter All the required sample collection site Details!!",
        "Warning Response"
      );
      return false;
    }
    if (pms_sample_gridStr) {
      if (pms_sample_gridStr.getCount() == 0) {
        toastr.warning(
          "Please add atleast one sample detail",
          "Warning Response"
        );
        return false;
      }
  ...
  showStructuredScreeningApplicationSubmissionWin: function (btn) {
    ...
    var mainTabPanel = this.getMainTabPanel(),
      ...
      table_name = getApplicationTable(module_id);
    if (valid == true || valid === true) {
      showPmsSamplesWorkflowSubmissionWin(
        application_id,
        application_code,
        table_name,
        "workflowsubmissionspmssamplesfrm",
        winWidth,
        storeID,
        analysis_type,
        "",
  ...
  showStructuredTCMeetingApplicationSubmissionWin: function (btn) {
    ...
    if (valid == true || valid === true) {
      if (table_name == "tra_surveillance_applications") {
        var wkf_stage_id = activeTab
          .down("hiddenfield[name=initialStageId]")
          .getValue();
        showWorkflowSubmissionWin(
          application_id,
          application_code,
  ...
  showStructuredAnalysisApplicationSubmissionWin: function (btn) {
    //conformatory
    Ext.getBody().mask("Please wait...");
    var mainTabPanel = this.getMainTabPanel(),
      winWidth = btn.winWidth,
  ...
  showStructuredPIRApplicationSubmissionWin: function (btn) {
    ...
    var mainTabPanel = this.getMainTabPanel(),
      ...
      analysis_type = activeTab
        .down("hiddenfield[name=analysis_type]")
        .getValue(),
      valid = true,
      storeID = getApplicationStore(module_id, section_id),
      table_name = getApplicationTable(module_id);
  ...
  showSurveillanceApplicationMoreDetails: function (btn) {
    ...
    if (sub_module_id == 37 || sub_module_id === 37) {
      this.showSurveillanceApplicationMoreDetailsUnstructured(
        application_id,
        application_code,
        applicant_id,
        ref_no,
      ...
      this.showSurveillanceApplicationMoreDetailsGeneric(
        application_id,
        application_code,
        applicant_id,
        ref_no,
  ...
  showSurveillanceApplicationMoreDetailsGeneric: function (
    application_id,
    application_code,
    applicant_id,
    ref_no,
      ...
      success: function (response) {
        ...
        if (success == true || success === true) {
          if (mainResults) {
            ...
            wizardPnl
              .down("combo[name=directorate_id]")
              .setValue(mainResults.directorate_id);
          }
  ...
  showSurveillanceApplicationMoreDetailsUnstructured: function (
    application_id,
    application_code,
    applicant_id,
    ref_no,
