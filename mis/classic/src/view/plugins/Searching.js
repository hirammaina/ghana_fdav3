Ext.define("Admin.view.plugins.Searching", {
  ...
  init: function (grid) {
    this.grid = grid;
    if (this.grid.rendered) this.onRender();
    else this.grid.on("render", this.onRender, this);
  },
  ...
  onRender: function () {
    ...
    var tb =
      "bottom" === this.position
        ? panel.getDockedItems('toolbar[dock="bottom"]')
    ...
    this.field = Ext.create("Ext.form.TwinTriggerField", {
      ...
      qtip: "ddd",
      selectOnFocus:
        undefined === this.selectOnFocus ? true : this.selectOnFocus,
      trigger1Cls: "x-form-clear-trigger",
      trigger2Cls: this.minChars ? "x-hidden" : "x-form-search-trigger",
      onTrigger1Click: Ext.bind(this.onTriggerClear, this),
      onTrigger2Click: this.minChars
        ? Ext.emptyFn
        : Ext.bind(this.onTriggerSearch, this),
          ...
          this.menu.items.each(function (item) {
            ...
            rv =
              rv instanceof Date
                ? Ext.Date.format(
                    rv,
                    this.dateFormat || r.fields.get(item.dataIndex).dateFormat
                  )
                : rv;
