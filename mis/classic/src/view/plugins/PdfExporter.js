...
Ext.define("Admin.view.plugins.PdfExporter", {
  ...
  saveDocumentAs: function (config) {
    ...
    for (var k = 0; k < exporter.data.columns.length; k++) {
      exporter.data.columns[k] = {
        style: "tableHeader",
        text: exporter.data.columns[k].text,
      };
    }
    ...
    for (var i = 0; i < dataRowLength; i++) {
      ...
      for (var j = 0; j < columnLength; j++) {
        exporter.data.groups[0].rows[i][j] =
          exporter.data.groups[0].rows[i][j].toString();
      }
