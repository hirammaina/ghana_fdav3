...
Ext.define("Admin.view.clinicaltrial.viewcontrollers.ClinicalTrialVctr", {
  ...
  navigateOnlineCtrRegDetails: function (button, wizardPanel, direction) {
    ...
    if (activeIndex == 1) {
      ...
      wizardPanel
        .down("button[name=save_clinicalseconaryids]")
        .setVisible(false);
      wizardPanel
        .down("button[name=save_clinicalstudyDesign]")
        .setVisible(false);
      wizardPanel
        .down("button[name=save_clinicaleligibilitycriteria]")
        .setVisible(false);
      wizardPanel
        .down("button[name=save_clinicalfundingsource]")
        .setVisible(false);
      wizardPanel
        .down("button[name=save_clinicalcollaborators]")
        .setVisible(false);
    } else if (activeIndex == 2) {
      wizardPanel
        .down("button[name=save_clinicalregdetails]")
        .setVisible(false);
      wizardPanel
        .down("button[name=save_clinicalseconaryids]")
        .setVisible(true);
  ...
  onNextCardClickOnlineDetails: function (btn) {
    var wizardPnl = btn.up("clinicaltrialonlineregistrypreviewwizard");
    if (!wizardPnl) {
      wizardPnl = btn.up(btn.wizardPnl);
    }
    wizardPnl.getViewModel().set("atBeginning", false);
  ...
  showGCPInspectionDetailsWizard: function (view, record) {
    ...
    this.fireEvent(
      "showGCPInspectionDetailsWizard",
      application_id,
      "gcpinspectionprocesswizard",
      app_inspection_id,
      application_code,
      applicant_id,
      ref_no,
  ...
  showGCPApprovlInspectionDetailsWizard: function (view, record) {
    ...
    this.fireEvent(
      "showGCPInspectionDetailsWizard",
      application_id,
      "gcpapprovalinspectionprocesswizard",
      app_inspection_id,
      application_code,
      applicant_id,
      ref_no,
  ...
  compareClinicalTrialApplicationDetails: function (item) {
    var comparePanel = Ext.widget("clinicaltrialcomparepanel"),
      ...
      portalStudySitesGrid = portalPanel.down(
        "onlineclinicaltrialstudysitesgrid"
      ),
      misStudySitesGrid = misPanel.down("clinicaltrialstudysiteswingrid"),
      portalInvestigatorsGrid = portalPanel.down(
        "onlineclinicaltrialotherinvestigatorsgrid"
      ),
      misInvestigatorsGrid = misPanel.down(
        "clinicaltrialotherinvestigatorswingrid"
      ),
    ...
    portalPanel
      .down("hiddenfield[name=application_id]")
      .setValue(portal_application_id);
    portalPanel
      .down("hiddenfield[name=application_code]")
      .setValue(application_code);
    portalPanel
    ...
    portalWizard
      .down("button[step=" + (portalWizard.items.length - 1) + "]")
      .setHidden(true);
  ...
  acceptPortalAmendedDetails: function (button) {
    Ext.MessageBox.confirm(
      "Confirm",
      "This will override MIS details with PORTAL details, Continue?",
      function (btn) {
        if (btn === "yes") {
  ...
  showAddClinicalTrialUnstructuredQueriesWin: function (btn) {
    ...
    if (app_status_id == 8 || app_status_id === 8) {
      //manager raise query
    ...
    if (app_status_id == 13 || app_status_id === 13) {
      //manager query response
  ...
  printMeetingDetails: function (btn) {
    ...
    if (!sm.hasSelection()) {
      toastr.warning(
        "No record selected. Please select a record(s) to to export!!",
        "Warning Response"
      );
      return;
    }
    Ext.MessageBox.confirm(
      "Confirm",
      "Are you sure you want to export the saved Meeting Application Details?",
      function (btn) {
        if (btn === "yes") {
          ...
          if (check) {
            url =
              "clinicaltrial/printApplicationMeetingDetails?selected=" +
            ...
            toastr.warning(
              "Some selections havent been saved",
              "Warning Response"
            );
            Ext.getBody().unmask();
          }
  ...
  showClinicalRegDataCleanUpWindow: function (btn) {
    var container = Ext.ComponentQuery.query("#contentPanel")[0],
      ...
      child = Ext.widget(btn.childXtype);
    child
      .down("clinicaltrialregistrydetailsfrm")
