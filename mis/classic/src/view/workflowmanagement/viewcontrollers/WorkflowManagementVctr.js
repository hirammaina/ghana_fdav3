 ...
 */
Ext.define(
  "Admin.view.workflowmanagement.viewcontrollers.WorkflowManagementVctr",
  {
    ...
    showEditWorkflowParamGridFrm: function (item) {
      //for tree panels
      //if (this.fireEvent('checkFullAccess') || this.fireEvent('checkWriteUpdate')) {
      var me = this,
        btn = item.up("button"),
    ...
    onProcessStageClick: function (view, record, item, index, e, eOpts) {
      ...
      checklistsGrid.setTitle(
        "Applicable Checklist Categories - " + stage_name + " Stage"
      );
      store.removeAll();
      store.load({
        params: { process_id: process_id, workflow_stage: stage_id },
      });
    },
    ...
    setWorkFlowNextStageDetails: function (cmb, newVal) {
      var form = cmb.up("form"),
        ...
        has_inspectionsche_assignment = record.get(
          "has_inspectionsche_assignment"
        ),
        has_firstassessment_assignment = record.get(
          "has_firstassessment_assignment"
        ),
        has_secondassessment_assignment = record.get(
          "has_secondassessment_assignment"
        ),
        has_assessreportreview_assignment = record.get(
          "has_assessreportreview_assignment"
        ),
        ...
        inspectionsche_assignmentcbo = form.down(
          "combo[name=inspection_schedulingstage_id]"
        ),
        firstassessment_assignmentcbo = form.down(
          "combo[name=firstassessment_stage_id]"
        ),
        secondassessment_assignmentcbo = form.down(
          "combo[name=secondassessment_stage_id]"
        ),
        assessreportreview_assignmentcbo = form.down(
          "combo[name=assessmentreport_reviewstage_id]"
        ),
        premisesvalidation_assignmentcbo = form.down(
          "combo[name=premisesvalidation_assignment_id]"
        );
      ...
      if (has_multistage_assignment == 1) {
        ...
        if (has_inspectionsche_assignment == 1) {
          ...
          form
            .down("combo[name=inspectionscheduling_respuser_id]")
            .setVisible(true);
        }
        ...
        if (has_secondassessment_assignment == 1) {
          ...
          form
            .down("combo[name=secondassessment_respuser_id]")
            .setVisible(true);
        }
        ...
        if (has_premises_validation == 1) {
          ...
          form
            .down("combo[name=premisesvalidation_respuser_id]")
            .setVisible(true);
        }
      } else {
        var fields = [
          screening_assignmentcbo,
          inspectionsche_assignmentcbo,
          firstassessment_assignmentcbo,
          secondassessment_assignmentcbo,
          assessreportreview_assignmentcbo,
        ];
        ...
        if (form.down("combo[name=screening_respuser_id]")) {
          form.down("combo[name=screening_respuser_id]").setVisible(false);
        }
        if (form.down("combo[name=inspectionscheduling_respuser_id]")) {
          form
            .down("combo[name=inspectionscheduling_respuser_id]")
            .setVisible(false);
        }
        if (form.down("combo[name=firstassessment_respuser_id]")) {
          form
            .down("combo[name=firstassessment_respuser_id]")
            .setVisible(false);
        }
        if (form.down("combo[name=secondassessment_respuser_id]")) {
          form
            .down("combo[name=secondassessment_respuser_id]")
            .setVisible(false);
        }
        if (form.down("combo[name=assessmentreportreview_respuser_id]")) {
          form
            .down("combo[name=assessmentreportreview_respuser_id]")
            .setVisible(false);
        }
      }
      /*mask = new Ext.LoadMask({
      ...
      if (form.down("hiddenfield[name=workflowaction_type_id]")) {
        form
          .down("hiddenfield[name=workflowaction_type_id]")
          .setValue(action_type_id);
      }
      if (form.down("hiddenfield[name=is_external_usersubmission]")) {
        form
          .down("hiddenfield[name=is_external_usersubmission]")
          .setValue(is_external_usersubmission);
      }

      if (form.down("hiddenfield[name=is_inspection_submission]")) {
        form
          .down("hiddenfield[name=is_inspection_submission]")
          .setValue(is_inspection_submission);
      }
      if (newVal && newVal > 0) {
        ...
        Ext.Ajax.request({
          ...
          success: function (response) {
            ...
            if (results) {
              ...
              if (form.down("combo[name=responsible_user]")) {
                form
                  .down("combo[name=responsible_user]")
                  .setValue(results.responsible_user_id);
              }

              if (has_multistage_assignment == 1) {
                screening_assignmentcbo.setValue(results.screening_stage_id);
                inspectionsche_assignmentcbo.setValue(
                  results.inspection_schedulingstage_id
                );
                firstassessment_assignmentcbo.setValue(
                  results.firstassessment_stage_id
                );
                secondassessment_assignmentcbo.setValue(
                  results.secondassessment_stage_id
                );
