...
Ext.define("Admin.view.surveillance.viewcontrollers.SurveillanceVctr", {
  ...
  showEditPmsProgram: function (item) {
    //wrong
  ...
  showAddPmsProgramPlanWinFrm: function (btn) {
    var me = this,
      ...
      program_id = containerPnl
        ...
        .down("hiddenfield[name=id]")
        .getValue(),
      program_implementation_id = containerPnl
      ...
      dosageFormsStr = childObject
        ...
        .getStore(),
      samplingSitesStore = childObject
        .down("combo[name=sampling_site_id]")
        .getStore(),
      packagingUnitsStore = childObject
        .down("combo[name=packaging_unit_id]")
        .getStore(),
      filters = {
        section_id: section_id,
      };
  ...
  onViewSurveillanceApplication: function (view, record) {
    ...
    if (is_locked == 1 || is_locked === 1) {
      toastr.warning(
        "This application is locked and cannot be accessed from here, use In-Tray to access the application!!",
        "Warning Response"
      );
      return false;
    }
  ...
  generateProductInformationReport: function (item) {
    ...
    var action_url =
      "reports/printProductInformationReport?application_id=" + sample_id;
  ...
  showEditSampleWindow: function (item) {
    ...
    var section_id = activeTab.down("hiddenfield[name=section_id]").getValue(),
      pms_program_id = activeTab
        .down("hiddenfield[name=program_id]")
        .getValue(),
      childObject = Ext.widget(childXtype),
      sampleDetailsFrm = childObject.down("form"),
      productsStr = childObject
        .down("#pmsprogramplan")
        .down("combo[name=product_id]")
        .getStore(),
    ...
    childObject
      .down("#sampledetailsfrm")
      .down("hiddenfield[name=isReadOnly]")
      .setValue(isReadOnly);
    childObject
      .down("pmssampleingredientsgrid")
      .down("hiddenfield[name=isReadOnly]")
      .setValue(isReadOnly);
  ...
  showPmsSampleLabRecommendation: function (item) {
    ...
    childObject
      ...
      .setValue(sample_application_code);
    childObject.down("hiddenfield[name=sample_id]").setValue(sample_id);
    childObject
      .down("hiddenfield[name=analysis_type_id]")
      .setValue(analysis_type);
    funcShowCustomizableWindow(
      "Application Ref Number: " + sample_refNo,
      winWidth,
      childObject,
      "customizablewindow"
    );
  ...
  showPmsSampleRecommendationWin: function (item) {
    ...
    childObject.down("hiddenfield[name=sample_id]").setValue(sample_id);
    if (childObject.down("hiddenfield[name=application_id]")) {
      childObject.down("hiddenfield[name=application_id]").setValue(sample_id);
    }
    if (childObject.down("hiddenfield[name=limssample_id]")) {
      childObject
        .down("hiddenfield[name=limssample_id]")
        .setValue(limssample_id);
    }

    if (childObject.down("hiddenfield[name=sample_appcode]")) {
      childObject
  ...
  savePIRRecommendation: function (btn) {
    ...
    if (!decision_id) {
      ...
      toastr.warning(
        "Please give PIR Recommendation to save!!",
        "Warning Response"
      );
      return false;
    }
  ...
  showProgramImplementationPlans: function (item) {
    ...
    pmsprogramimplpanel
      .down("displayfield[name=program_implementation]")
      .setValue(record.get("program_implementation"));
    pmsprogramimplpanel
      .down("hiddenfield[name=program_implementation_id]")
      .setValue(record.get("id"));
    pmsprogramimplpanel
      .down("displayfield[name=implementation_timeline]")
      .setValue(
        record.get("implementationstart_date") +
          " -" +
          record.get("implementationend_date")
      );
    pmsprogramimplpanel
      .down("displayfield[name=program]")
      .setValue(record.get("program_name"));

    var plansGrid = pmsprogramimplpanel.down("pmsprogramplansgrid"),
      dosageFormCol = plansGrid
        .getColumnManager()
        .getHeaderByDataIndex("dosage_form"),
      productFormCol = plansGrid
        .getColumnManager()
        .getHeaderByDataIndex("product_form"),
      deviceTypeCol = plansGrid
        .getColumnManager()
