Ext.define('Admin.view.summaryreport.registration.viewControllers.RegistrationReportViewCtr', {
    ...
    exportProductRevenueRegister: function(btn) {
      var panel=btn.up('panel'),
      filter=panel.down('form'),
      grid = panel.down('revproductregistergrid'),
      ...
      paid_fromdate = filter.down('datefield[name=paid_fromdate]').getValue(),
      paid_todate = filter.down('textfield[name=paid_todate]').getValue();
      paid_fromdate = Ext.Date.format(paid_fromdate,'Y-m-d');   
      paid_todate = Ext.Date.format(paid_todate,'Y-m-d'); 
      ...
      Ext.Ajax.request({
          url: 'registers/exportProductRevenueRegister',
          method: 'GET',
          headers: {
      ...
      },loadRevenueProductRegisterFilters: function (btn) {
        var grid = btn.up('form'),
          ...
          panel = grid.up('panel'),
          gridStr = panel.down('revproductregistergrid').getStore();
      ...
      printProductRevenueGazzete: function(btn) {
      var panel=btn.up('panel'),
      filter=panel.down('form'),
      grid = panel.down('revproductregistergrid'),
      filterfield = grid.getPlugin('filterfield');       
      ...
      Ext.Ajax.request({
         url: 'registers/checkPrintProductRevenueGazzete',
          method: 'GET',
          headers: {
               'Authorization':'Bearer '+access_token
                   },
          ...
          success: function (response) {
              var resp = JSON.parse(response.responseText);
               if (resp.status == 'sucesss' || resp.status === 'success' ) {
                print_report('registers/printProductGazzete?sub_module_id='+sub_module_id+'&section_id='+section_id+'&paid_todate='+paid_todate+'&paid_fromdate='+paid_fromdate+'&module_id='+module_id+'&filter='+encodeURIComponent(JSON.stringify(filter_array)));
              }
              else{
