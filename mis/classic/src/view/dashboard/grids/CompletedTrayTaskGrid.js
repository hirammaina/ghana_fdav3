 ...
 */
Ext.define('Admin.view.dashboard.grids.CompletedTrayTaskGrid', {
    ...
    bbar: [{
            xtype: 'pagingtoolbar',
            displayInfo: true,
            store: 'completedtraytaskstr',
            width: '60%',
            beforeLoad: function () {
                var store = this.getStore(),
                    grid = this.up('grid'),
                   // pnl = grid.up('intraypnl'),
                    is_receipting_stage = grid.is_receipting_stage,
                    section_id = grid.down('combo[name=section_id]').getValue(),
                    module_id = grid.down('combo[name=module_id]').getValue(),
