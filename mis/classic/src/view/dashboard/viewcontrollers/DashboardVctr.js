Ext.define("Admin.view.dashboard.viewcontrollers.DashboardVctr", {
      ...
      success: function (response) {
        ...
        if (resp.has_fasttrack_applications) {
          ...
          funcShowCustomizableWindow(
            "Submitted & Assigned Fast Track Applications",
            "86%",
            fasttrackappgrid,
            "customizablewindow"
          );
          ...
          toastr.error(
            "There is no assigned Fast Track application in your account",
            "Error Response"
          );
        }
      },
  ...
  loadApplicationAssaignmentTab: function (me) {
    ...
    Ext.Ajax.request({
      ...
      error: function (jqXHR, textStatus, errorThrown) {
        toastr.error(
          "Error checking assaingment: " + errorThrown,
          "Error Response"
        );
      },
    });
  ...
  viewAssaignedApplications: function (btn) {
    ...
    funcShowCustomizableWindow(
      user_name + " Assaigned Applications In " + stage_name + " stage",
      "86%",
      grid,
      "customizablewindow"
    );
  ...
  viewCOmpletedAssignmentApplications: function (btn) {
    ...
    grid.down("hiddenfield[name=stage_id]").setValue(record.get("stage_id"));
    grid
      .down("hiddenfield[name=released_by]")
      .setValue(record.get("released_by"));

    funcShowCustomizableWindow(
      user_name + " Completed Applications In " + stage_name + " stage",
      "86%",
      grid,
      "customizablewindow"
    );
  ...
  reAssaignpplications: function (btn) {
    ...
    form
      ...
      .setValue(record.get("current_stage"));
  ...
  exportDashboard: function (btn) {
    ...
    if (grid.down("combo[name=zone_id]")) {
      var zone_id = grid.down("combo[name=zone_id]").getValue(),
        application_status_id = grid
          .down("combo[name=application_status_id]")
          .getValue();
    } else {
      var zone_id = null,
    ...
    var str =
      "dashboard/exportDashboard?type=" +
      type +
      "&is_internaluser=" +
      is_internaluser +
      "&section_id=" +
      ...
      application_status_id +
      "&filter=" +
      encodeURIComponent(filter_array);
