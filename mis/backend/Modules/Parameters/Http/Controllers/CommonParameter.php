    ...
    public function __construct()
    {
        ...
        $this->invoker = [
            ...
            "save-region" => function ($request) {
                $validator = $this->validateParameterRequest(
                    $request,
                    "country_id"
                );
            ...
            "save-costsubcategory" => function ($request) {
                ...
                return CostSubCategory::saveData(
                    $request,
            ...
            "get-portalparameters" => function ($start, $limit, $doRetrieveAll, $filter = null) {
                return PortalParameter::getData(
                    $start,
                    $limit,
                    $doRetrieveAll,
            ...
            "get-costsubcategory" => function ($start, $limit, $doRetrieveAll, $filter = null) {
                ...
                return CostSubCategory::getData(
                    $start,
                    $limit,
                    $doRetrieveAll,
                    $filter != null ? $this->parseFilter($filter) : null
            ...
            "get-transactiontype" => function ($start, $limit, $doRetrieveAll, $filter = null) {
                return TransactionType::getData(
                    $start,
                    $limit,
                    $doRetrieveAll,
                    $filter != null ? $this->parseFilter($filter) : null
            ...
            "get-paymentinterval" => function ($start, $limit, $doRetrieveAll, $filter = null) {
                return PaymentInterval::getData(
                    $start,
                    $limit,
                    $doRetrieveAll,
                    $filter != null ? $this->parseFilter($filter) : null
        ...
        try {
            ...
            if ($table_name == 'par_business_types') {
                $qry = DB::connection($db_con)
                    ...
                    ->leftJoin('tra_sectionsbusiness_types as t2', 't1.id', 't2.business_type_id')
                ...
                unset($filters['business_type_id']);

                $qry = DB::connection($db_con)
                    ->table($table_name . ' as t1')
                    ->join('par_business_types as t3', 't1.business_type_id', '=', 't3.id')
                    ->join('tra_sectionsbusiness_types as t4', 't3.id', '=', 't4.business_type_id')
                    ->join('par_sections as t2', 't4.section_id', '=', 't2.id')
                    ->select('t1.*', 't4.section_id', 't2.name as section_name', 't3.name as business_type_name')
                    ->where('t1.business_type_id', $business_type_id);
            ...
            } else if ($table_name == 'par_expirynotification_timespan' || $table_name == 'par_auditreport_config' || $table_name == 'par_service_types'  || $table_name == 'par_variation_reportingtypes') {
                $qry = DB::connection($db_con)
                    ->table($table_name . ' as t1')
                ...
                $qry = DB::connection($db_con)
                    ...
                    ->leftJoin('par_product_categories as t5', 't1.product_category_id', '=', 't5.id')
                    ->leftJoin('par_sections as t6', 't1.section_id', '=', 't6.id')
                    ->select('t1.*', 't3.name as module_name', 't6.name as section_name', 't4.name as sub_module_name', 't5.name as product_category');
            } else if ($table_name == 'par_variation_subcategories'  || $table_name == 'par_variationconditions_details' || $table_name == 'par_variationsupporting_datadocs') {
                $qry = DB::connection($db_con)
                    ->table($table_name . ' as t1')
                    ->leftJoin('modules as t3', 't1.module_id', '=', 't3.id')
                    ->leftJoin('sub_modules as t4', 't1.sub_module_id', '=', 't4.id')
                    ->leftJoin('par_product_categories as t5', 't1.product_category_id', '=', 't5.id')
                    ->leftJoin('par_sections as t6', 't1.section_id', '=', 't6.id')
                    ->leftJoin('par_variation_categories as t7', 't1.variation_category_id', '=', 't7.id')

                    ->select('t1.*', 't3.name as module_name', 't6.name as section_name', 't7.name as variation_category', 't4.name as sub_module_name', 't5.name as product_category');
                ...
                $qry = DB::connection($db_con)
                    ...
                    ->leftJoin('par_sections as t6', 't1.section_id', '=', 't6.id')
                    ->leftJoin('par_variation_categories as t7', 't1.variation_category_id', '=', 't7.id')
                    ->leftJoin('par_variation_subcategories as t8', 't1.variation_subcategory_id', '=', 't7.id')

                    ->select('t1.*', 't3.name as module_name', 't6.name as section_name', 't7.name as variation_category', 't8.name as variation_subcategory', 't4.name as sub_module_name', 't5.name as product_category');
                ...
                $qry = DB::connection($db_con)
                    ...
                    ->leftJoin('par_variation_categories as t7', 't1.variation_category_id', '=', 't7.id')
                    ->leftJoin('par_variation_subcategories as t8', 't1.variation_subcategory_id', '=', 't7.id')
                    ->leftJoin('par_variation_description as t9', 't1.variation_description_id', '=', 't9.id')

                    ->select('t1.*', 't9.name as variation_description', 't3.name as module_name', 't6.name as section_name', 't7.name as variation_category', 't8.name as variation_subcategory', 't4.name as sub_module_name', 't5.name as product_category');
                ...
                $qry = DB::connection($db_con)
                    ...
                    ->select('t1.*', 't3.name as currency_name', DB::raw("TOTAL_WEEKDAYS(now(), t1.dola) as number_of_days"));
    ...
    public function getelementcost(request $request)
    {
        ...
        try {
            $qry = DB::table('tra_element_costs as t1')
                ...
                ->LeftJoin('irembo_feescode_details as t13', 't1.irembo_feescode_id', 't13.id')
                ->select(
                    't1.*',
                    't1.id as element_costs_id',
                    't11.name as section_name',
                    't4.name as currency_name',
                    'a7.section_id',
                    't4.id as currency_id',
                    't2.name as element',
                    't13.product_code as irembo_productcode',
                    't13.product_name as irembo_productname',
                    't13.product_amount',
                    't4.name as currency',
