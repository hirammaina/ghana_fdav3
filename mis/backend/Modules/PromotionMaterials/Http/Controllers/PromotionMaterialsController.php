    ...
    public function saveApplicantDetails(Request $request)
    {
        try {
            ...
            $promotion_material_data = array(
                ...
                'promotionmaterial_category_id' => $request->promotionmaterial_category_id,
                'prommaterial_bookcatalogues_id' => $request->prommaterial_bookcatalogues_id,
            ...
            if (validateIsNumeric($active_application_id)) {
                ...
                if (is_array($promotions_material_id)) {
                    foreach ($promotions_material_id as $promotion_material_id) {
                        ...
                        $material_detailsdata[] = array(
                            'material_id' => $promotion_material_id,
                            'application_id' => $active_application_id,
                            'created_by' => $user_id,
                            'created_on' => Carbon::now()
                        );
                ...
                if (is_array($advertisement_channels_id)) {
                    foreach ($advertisement_channels_id as $advertisement_channel_id) {
                        ...
                        $material_detailsdata[] = array(
                            'advertisement_channel_id' => $advertisement_channel_id,
                            'application_id' => $active_application_id,
                            'created_by' => $user_id,
                            'created_on' => Carbon::now()
                        );
        ...
        try {
            ...
            $qry = $portal_db->table('wb_promotion_adverts_applications as t1')
                ...
                ->select(
                    ...
                    't4.is_manager_query',
                    't6.name as rejection_reason',
                    't5.remark as rejection_remark',
                    't5.created_on as rejection_date'
                )
        ...
        try {
            ...
            $qry1->join('wb_trader_account as t3', 't1.applicant_id', '=', 't3.id')
                ->select(
                    ...
                    't3.name as applicant_name',
                    't3.contact_person',
                    't9.name as applicant_sponsor_name',
                    't3.tin_no',
                    't3.country_id as app_country_id',
                    't3.region_id as app_region_id',
                    't3.district_id as app_district_id',
            ...
            $query = DB::table('tra_premises as t1')
                ->select(
                    't1.*',
                    ...
                    't1.telephone as app_telephone',
                    't1.email as app_email',
                    't1.website as app_website'
                )
                ->where(array('t1.id' => $local_agent_id));
        ...
        try {
            $qry = DB::table('tra_promotion_adverts_applications as t1')
                ...
                ->select(
                    ...
                    't5.decision_id',
                    't1.id as active_application_id',
                    DB::raw("CONCAT_WS(' ',decrypt(t8.first_name),decrypt(t8.last_name)) as from_user,CONCAT_WS(' ',decrypt(t9.first_name),decrypt(t9.last_name)) as to_user,t12.name as applicant_sponsor_name")
                )
        ...
        try {
            $qry = DB::table($table_name . ' as t1')
                ...
                ->select(
                    ...
                    DB::raw("CONCAT_WS(' ',decrypt(t11.first_name),decrypt(t11.last_name)) as from_user,CONCAT_WS(' ',decrypt(t12.first_name),decrypt(t12.last_name)) as to_user,t13.name as applicant_sponsor_name")
                )
