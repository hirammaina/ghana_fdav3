...
class ClinicalTrialController extends Controller
{
    ...
    public function getClinicalTrialsList(Request $request)
    {
        ...
        try {
            $qry = DB::table('tra_clinical_trial_applications as t2')
                ...
                ->leftJoin('tra_approval_recommendations as t6', 't2.permit_id', '=', 't6.id')
    ...
    public function prepareNewClinicalTrialManagerMeetingStage(Request $request)
    {
        ...
        try {
            $qry = DB::table($table_name . ' as t1')
                ->Join('tc_meeting_applications as t2', function ($join) use ($application_code) { //Job changed to Join since null id causes ext js generate falke id
    ...
    public function getClinicalStudySites(Request $request)
    {
        ...
        try {
            ...
            if (validateIsNumeric($application_id)) {
                ...
                $results = $qry->get();
                if ($results->count() == 0) {
                    $results = DB::table('study_sites as t1')
                        ->select('t1.*')
                        ->get();
                }
            }
            ...
            if (validateIsNumeric($application_id)) {
                $qry = DB::table('clinical_trial_investigators as t1')
                    ...
                    ->select(
                        ...
                        't5.category_name as category',
                        't6.name as study_site'
                    );
                //if (isset($application_id) && $application_id != '') {
                $qry->where('t1.application_id', $application_id);
                //}
        ...
        try {
            ...
            if (validateIsNumeric($application_id)) {
                $qry = DB::table($table_name . ' as t1')
                    ...
                    ->select(DB::raw('t1.*,CONCAT(t1.product_strength,t6.name) as product_strength_txt,t2.category_name,t3.name as common_name,t4.name as dosage_form,t5.name as admin_route,t8.name as market_location,t9.name as classification_name'))
                    ->where('t1.application_id', $application_id);
                $results = $qry->get();
            }
    ...
    public function getstrProgressReportingManagerApplicationsGeneric(Request $request)
    {
        ...
        try {
            ...
            $qry->select(
                ...
                'actualstudy_date',
                't3.name as applicant_name',
                't12.name as evaluation_recommendation',
                't4.name as application_status',
                't6.name as approval_status',
                't5.decision_id',
                't1.id as active_application_id',
        ...
        try {
            $qry = DB::table($table_name . ' as t1')
                ...
                ->select(
                    ...
                    't7.name as sponsor',
                    't8.name as investigator',
                    't12.name as assessment_recomm',
                    't13.name as audit_recomm'
                )
    ...
    function getGCPInspectionScheduleingDetails($req, $where, $table_name)
    {
        ...
        try {
            ...
            $qry = DB::table($table_name . ' as t1')
                ...
                ->leftJoin('tra_submissions  as t18', 't1.application_code', '=', 't18.application_code')
    ...
    public function getClinicalTrialCtrgcpInspectionDetails(Request $req)
    {
        ...
        $where = array('t7.inspection_id' => $inspection_id, 't18.current_stage' => $workflow_stage, 't18.isDone' => 0);
        ...
        try {
            ...
            if (!validateIsNumeric($id)) {
                ...
                if ($record) {
                    ...
                    $application_params['view_id'] = $view_id;
                    $application_params['application_status_id'] = $application_status->status_id;
                    //initializeApplicationDMS($record->section_id, $record->module_id, $sub_module_id, $application_code, $ref_number, $user_id);//Job to reinstate 28.02.24
                    $application_params['permit_id'] = null;
                    unset($application_params['id']);
    ...
    public function printApplicationMeetingDetails(Request $req)
    {
        ...
        $qry = DB::table($table_name . ' as t1')
            ...
            ->join('wf_workflow_actions as t13', 't11.recommendation_id', '=', 't13.id')
            ->select(
                't1.tracking_no as Tracking_No',
                't1.reference_no as Reference_No',
                't3.name as Applicant_Name',
                't4.name as Application_Status',
                't7.name as Sponsor',
                't8.name as Investigator',
                't12.name as Assessment_Recommendation',
                't13.name as Audit_Recommendation'
            )
            ->where('t1.workflow_stage_id', $workflow_stage)
            ->whereNotIn('t1.application_code', function ($query) use ($table_name, $workflow_stage, $meeting_id) {
                $query->select(DB::raw('t8.application_code'))
        ...
        try {
            ...
            if (is_array($disease_being_studied)) {
                foreach ($disease_being_studied as $disease_id) {
                    ...
                    $disease_beingdata[] = array(
                        'disease_being_studied' => $disease_id,
                        'application_id' => $application_id,
    ...
    public function save_clinicalstudyDesign(Request $req)
    {
        ...
        $data = array(
            'intervention_assignment_id' => $req->intervention_assignment_id,
    ...
    public function save_clinicalseconaryids(Request $req)
    {
        ...
        $app_data = array(
            'is_secondaryid_applicable' => $req->is_secondaryid_applicable,
    ...
    public function onSaveInterventionDetails(Request $req)
    {
        ...
        $app_data = array(
            'intervention_type_id' => $req->intervention_type_id,
    ...
    public function onSaveOutcomesDetails(Request $req)
    {
        ...
        $app_data = array(
            'outcome_type_id' => $req->outcome_type_id,
    ...
    public function save_clinicalcollaborators(Request $req)
    {
        ...
        $app_data = array(
            'has_collaborators' => $req->has_collaborators,
            'name' => $req->name,
            'physical_address' => $req->physical_address,
            'country_id' => $req->country_id,
            'region_id' => $req->region_id,
    ...
    public function onSaveContactPersonDetails(Request $req)
    {
        ...
        $app_data = array(
            'contactperson_role_id' => $req->contactperson_role_id,
            'name' => $req->name,
            'physical_address' => $req->physical_address,
            'country_id' => $req->country_id,
            'region_id' => $req->region_id,
    ...
    public function save_clinicalfundingsource(Request $req)
    {
        ...
        $app_data = array(
            'is_funding_receivied' => $req->is_funding_receivied,
