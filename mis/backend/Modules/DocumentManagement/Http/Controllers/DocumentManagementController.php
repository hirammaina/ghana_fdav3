...
class DocumentManagementController extends Controller
{
    ...
    public function getProcessApplicableDocRequirements(Request $req)
    {
        ...
        try {
            ...
            if ($docType_id == 2) {
                ...
                if ($module_id == 2 &&  $docType_id == 2) {

                    $prem_rec = DB::table('tra_premises_applications as t1')
                        ->select('t2.business_type_id')
    ...
    function getApplicationDocumentsUploads($req)
    {
        ...
        //
        $business_type_id = 0;
        $premisesbusiness_type_id = null;
        if ($module_id == 2) {
            $prem_rec = DB::table('tra_premises_applications as t1')
                ->select('t2.business_type_id')
                ->join('tra_premises as t2', 't1.premise_id', 't2.id')
                ->where('application_code', $application_code)->first();
            $premisesbusiness_type_id = $prem_rec->business_type_id;
        }
        ...
        if (validateIsNumeric($doc_type_id) && $doc_type_id == 4) {
        ...
        foreach ($docTypes as $doc) {
            if ($doc == 2) {
                if (validateIsNumeric($premisesbusiness_type_id)) {
                    $business_type_id = $premisesbusiness_type_id;
                }
            }
        }
        if (validateIsNumeric($business_type_id)) {
        ...
        if (validateIsNumeric($portal_uploads)) {
            $where['portal_uploadable'] = 1;
        }
        ...
        if (validateIsNumeric($parent_id)) {
            ...
            if (validateIsNumeric($business_type_id)) {
                ...
                $doc_requirments->where('business_type_id', $business_type_id);
            }
            ...
            foreach ($doc_requirments as $doc_req) {
                ...
                if (validateIsNumeric($business_type_id)) {
                    ...
                    $qry->where('t2.business_type_id', $business_type_id);
                }
        ...
        try {
            ...
            if ($app_rootnode) {
                ...
                if ($req->hasFile('uploaded_doc')) {
                    ...
                    $document_path = $destination . $savedName;
                    //check if tje dpcument type has been mapped and not autoCreate the folder
                    $document_type = getParameterItem('par_document_types', $document_type_id);
    ...
    //the details 
    public function getDocumentDownloadurl(Request $req)
    {
        try {
            ini_set('memory_limit', '-1');
            $user_id = $this->user_id;
    ...
    public function getDocumentArchive(Request $req)
    {
        try {
            ...
            $selected_archives = json_decode(base64_decode($selected));
            $doc_url = array();
            foreach ($selected_archives as $selected_archive) {
                ...
                $res = $this->getDocumentDownloadurl($request);
    ...
    public function uploadLargeFiles(Request $request)
    {
        ...
        if (!recordExists('tra_application_documentsdefination', $where_app)) {
            initializeApplicationDMS(7, $module_id, $sub_module_id, $application_code, 'Doc' . rand(0, 1000), '');
        }
    ...
    public function saveUploadedApplicationPayments(Request $req)
    {
        try {
            ...
            if ($req->hasFile('uploaded_doc')) {
                ...
                if ($trans_date != null) {
                    $trans_date = Carbon::parse($trans_date);
                }
                $payment_data = array(
                    'application_code' => $application_code,
                    'trans_date' => $trans_date instanceof Carbon ?   formatDate($trans_date) : null,
