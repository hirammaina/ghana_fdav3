...
var KicaCloudLoading = function () {
  ...
  this.init = function () {
    if (!loading.isAppended) loading.html.appendTo("body");
...
var KicaCloudDialog = function () {
  ...
  this.init = function (title, msg, destroyUI) {
    if (!dialog.isAppended) dialog.html.appendTo("body");
    ...
    dialogBtnClick = function () {
      dialog.destroy();
      if (destroyUI) $("#kicaCloudControlWrap").remove();
    };
    dialog.html
      .find("#kicaCloudDialogFooter button")
      .on("click", dialogBtnClick);
...
var KicaCloudUI = function () {
  var self = this;
  (self.isGenerate = false), (self.opt = {}), (self.wrap = null);
  ...
  var htmlElements = {
    ...
    common: function (title, type) {
      var html =
        '<div id="kicaCloudControlWrap" ' +
        (type == "LOGINONLY" ? "style='display:none !important;'" : "") +
        ">";
      html += '<input type="hidden" id="cloudPKI_access_token">';
      html +=
        '<input type="hidden" id="cloudPKI_access_type" value="' + type + '">';
      ...
      if (type !== "CERTLIST") {
        ...
        html +=
          '<label class="titIcon"><span>Enter the certificate ' +
          (type == "CHANGEPASSWORD" ? "old" : "") +
          " password.</span></label>";
        html +=
          '<input autocomplete="off" type="password" id="certificatePasswordInput" maxlength="60" name="cert_pw" placeholder="You have only 3 times chances to enter your correct password.">';
        html +=
          '<span class="cloudPasswordAlert">If you have lost your certificate password, you have to apply for certificate re-issuance at the front system.</span>';
        html += "</div>";
      }
      html += '<div id="certificateButtonWrap">';
      if (type !== "CERTLIST") {
        html +=
          '<button type="button" id="cloudui-confirm-btn">Confirm</button>';
      }
    ...
    evt.drawContent = function (result) {
      ...
      $.each(result.result, function (k, v) {
        contentHtml +=
          ...
          "' data-revoked='" +
          v.revokeYn +
          "' onclick='KicaCloudUIEvent.certificateSelect(this, \"" +
          evt.type +
          "\")'>";
        contentHtml += "<td>";
        contentHtml += v.name;
