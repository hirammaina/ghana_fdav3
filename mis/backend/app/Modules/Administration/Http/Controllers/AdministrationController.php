...
public function getSystemNavigationMenuItems(Request $request)
    ...
    function getSystemMenuItem($level = 0, $parent_id = 0)
    {
        ...
        $groups = rtrim(convertArrayToString(getUserGroups($user_id)),',');
		...
		if($groups ==1){
            ...
            if($groups!=="")
            {
                $statement="((tra_permissions.group_id in (".$groups.") ";

            }
            if($statement!=="")
            {
                $statement="and tra_permissions.status = 1 and  tra_permissions.accesslevel_id != 1) or (par_menus.permission_based_access = 2))";
            }else{
                $statement="((tra_permissions.status = 1 and  tra_permissions.accesslevel_id != 1) or (par_menus.permission_based_access = 2))";
            }
            
            
			$qry->leftJoin('tra_permissions', 'par_menus.id', '=', 'tra_permissions.menu_id')
            ->whereRaw(DB::raw($statement));
				//->whereRaw(DB::raw("((tra_permissions.group_id in (".$groups.") and tra_permissions.status = 1 and tra_permissions.accesslevel_id != 1) or (par_menus.permission_based_access = 2))"));
   ...
   public function mapGroupToStage(Request $req){
        ...
        try{
           ...
           // foreach ($stage_idArray as $stage_id) {
			foreach ($stage_idArray as $stage) {
               $data = array('stage_id'=>$stage->stage_id,'access_level_id'=>$stage->access_level_id,  'group_id'=>$group_id);
                  $record = DB::table('wf_stages_groups')
                                    ->where(['stage_id'=>$stage->stage_id, 'group_id'=>$group_id])
                                    ->first();
                  if($record){
                        DB::table('wf_stages_groups')->where(['stage_id'=>$stage->stage_id, 'group_id'=>$group_id])->update($data);
                        $res = array(
                            'success' => true,
                    ...
                    $res = insertRecord('wf_stages_groups', $data, $user);
