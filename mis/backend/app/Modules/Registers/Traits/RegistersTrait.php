...
namespace App\Modules\Registers\Traits;
...
trait RegistersTrait
{
   ...
   public function DateFilter(request $req){
    ...
    } public function PaymentDateFilter(request $req){
        $paid_fromdate=$req->paid_fromdate;
        $paid_todate=$req->paid_todate;
        $where_raw=array();
    
        if($paid_fromdate != '' && $paid_todate != ''){
           $where_raw[]="date_format(date_filter, '%Y%-%m-%d') BETWEEN '".formatDate($paid_fromdate)."' AND '".formatDate($paid_todate)."'";
          }
...
public function DateFilterImportExport(request $req){
   ...
   public function getTableName($module){
     ...
     public function getProductRevenueRegister($table,$table2,$field,$filterdata,$subfilterdata,$datefilter){

        $qry=DB::table($table.' as t1')
           ->join('tra_payments as t70','t1.application_code','t70.application_code')
        ...
        if($filterdata!=''){
         ...
         //filter by date
        if($datefilter!=''){
          $datefilter = str_replace('date_filter','t70.trans_date',$datefilter);
