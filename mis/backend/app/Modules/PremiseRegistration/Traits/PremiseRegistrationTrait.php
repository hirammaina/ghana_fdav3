...
trait PremiseRegistrationTrait
{
    ...
    public function getPremiseApplicationReferenceCodes($application_details)
    {
        ...
        $business_type_id = $application_details->business_type_id;
        ...
        $premise_type_code = getSingleRecordColValue('par_business_types', array('id' => $business_type_id), 'code');
        $codes_array = array(
            'section_code' => $section_code,
            'year_code' => $registration_year,
            'month_code' => $month_code,
            'premise_type_code' => $premise_type_code,
            'zone_code' => $zone_code
        );
    ...
    public function savePremiseApplicationRecommendationDetails(Request $request,$document_types)
    {
        ...
        $qry = DB::table($table_name.' as t1')
			...
			->leftJoin('tc_recommendations as t3', 't1.application_code', 't3.application_code')
			->select('t1.*', 't2.business_type_id', 't3.certificate_expiry_date')
        ...
        try {
            DB::transaction(function () use ($qry, $application_id, $application_code, $table_name, $request, $app_details,$document_types, &$res) {
				...
				$certificate_expiry_date = $app_details->certificate_expiry_date;
				if( $expiry_date != ''){
				...
				if($decision_id == 1){
				 ...
				 $codes_array = $this->getPremiseApplicationReferenceCodes($app_details);
                ...
                if (validateIsNumeric($id)) {
                    ...
                    if ($decision_id == 1) {
                        ...
                        //permit'appvalidity_status_id' => 2
                        if ($prev_decision_id != 1) {
							if($permit_no == ''){
								...
								//$permit_no = generatePremisePermitNo($app_details->zone_id, $app_details->section_id, $table_name, $user_id, $ref_id ,$sub_module_id);
								 $certificate_data = generateApplicationCertificateNumber($application_id, $table_name, $sub_module_id,  2, $codes_array, $process_id, 0, $user_id,$module_id,$section_id);
								 if ($certificate_data['success'] == false) {
									echo json_encode($certificate_data);
									exit();
								}
								
								$permit_no = $certificate_data['certificate_no'];
                            ...
                            $params['permit_no'] = $permit_no;
                            $params['certificate_no'] = $permit_no;
 ...
 $params['appvalidity_status_id'] = 4;
				...
				$premise_reg_no = '';
                    if ($decision_id == 1) {
                        ...
                        $validity_status_id = 2;
                        $registration_status_id = 2;
                        //permits'application_status_id' => 6
						if($permit_no == ''){
			...
			if($decision_id == 1){
					   $res_cert = $this->funcGeneraPremisesertificate($request,$application_code,$sub_module_id);
					  
				 }
            \App::call('App\Modules\AuditReport\Http\Controllers\AuditReportController@logPremiseApplicationDetails',[$application_id]);
	...
	public function funcGeneraPremisesertificate($request,$application_code,$sub_module_id){
				
		$attachement_name = 'Premises_Certificate';
		$document_root = $_SERVER['DOCUMENT_ROOT'];
		$digital_signature_password = $request->digital_signature_password;
		$document_name = $attachement_name.$application_code.'.pdf';
		$upload_directory =  $document_root.'/mis/'.env('DOC_ROOTUPLOAD').$document_name;
		$doc_path = env('DOC_ROOTUPLOAD').$document_name;
		//generation or the certificte 
		if($request->digital_signature_password != ''){
			$this->printPremisesCertificateLetter($request,'','notify',$upload_directory);
			$digtal_certificate = funcDigitalSignatureHelperCall($document_name,$doc_path,$upload_directory,$digital_signature_password,$request,$application_code,$this->user_id);
			if($digtal_certificate['success']){
				$base64_endcodefile = $digtal_certificate['base64_endcodefile'];
				$premiseUpdateParams = array('base64_endcodefile'=>$base64_endcodefile);
				  DB::table('tra_approval_recommendations')
