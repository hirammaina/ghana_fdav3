...
namespace App\Modules\Importexportpermits\Http\Controllers;
        ...
        try {
            ...
            $qry1->join('wb_trader_account as t3', 't1.applicant_id', '=', 't3.id')
                ->select('t1.*','q.name as application_status', 't2.id as active_application_id',
                    't3.name as applicant_name', 't3.contact_person',
                    't3.tin_no', 't3.country_id as app_country_id', 't3.region_id as app_region_id', 't3.district_id as app_district_id', 't3.physical_address as app_physical_address',
                    't3.postal_address as app_postal_address', 't3.telephone_no as app_telephone', 't3.fax as app_fax', 't3.email as app_email', 't3.website as app_website', 't2.*', 't2.id as poe_application_id', 't2.port_id as poeport_id', 't1.custom_declaration_no', 't1.port_id'
                    );
    ...
    public function getImportExportManagerReviewApplications(Request $req)
    {
        ...
        try {
            ...
            $qry = DB::table($table_name . ' as t1')
                ...
                ->select('t1.*', 't3.name as applicant_name','t5.name as premises_name', 't5.physical_address as prem_physical_address', 't5.premise_reg_no' , 't9.name as special_caseapproval', 't8.decision_id as specialcaseapproval_id','t4.name as application_status','t7.id as recommendation_id', 't6.name as recommendation','t6.name as approval_status','t15.name as screening_recommendation', 't18.name as premises_validation_recommendation','t20.name as products_validation_recommendation',
                    't1.id as active_application_id',DB::raw("t10.date_received, CONCAT_WS(' ',decryptVal(t12.first_name),decryptVal(t12.last_name)) as from_user,t16.name as process_name, t11.name as workflow_stage"))
...
public function getNarcoticspermitsproductsDetails(Request $req){
 ...
 //validate if the tra number has been used
						...
						if($check_trnumber){
							...
							$res = array('success'=>false, 
										 'message'=>'Permit with the same RRA Registration number has already been entered under Tracking Number '.$tracking_no.', kinldy verify with the ICT department.');
							  return \response()->json($res);
						}
    ...
    try {
        ...
        $qry = DB::table('tra_importexport_applications as t1')
            ...
            ->select('t1.*', 't1.id as active_application_id','t1.id as application_id', 't1.application_code',
                ...
                't3.tin_no', 't3.country_id as app_country_id', 't3.region_id as app_region_id', 't3.district_id as app_district_id', 't3.physical_address as app_physical_address',
                't3.postal_address as app_postal_address', 't3.telephone_no as app_telephone', 't3.fax as app_fax', 't3.email as app_email', 't3.website as app_website',
                't3.name as applicant_name', 't4.name as sender_receiver_name', 't7.permit_no',DB::raw("date_format(t7.expiry_date, '%m/%d/%Y') as expiry_date, 'Approved' as application_status"))
