...
//allowed_extensions
        ...
        try {
            $results = DB::table('tra_documentupload_requirements as t1')
                ...
                ->leftJoin('par_business_types as t7', 't1.business_type_id', '=', 't7.id')
                ->select(DB::raw("t1.*,0 as leaf,t1.name as mtype,t7.name as business_type, t2.name as document_type,t6.name as prodclass_category, t3.name as module_name, t4.name as sub_module, t5.name as section_name, (select group_concat(concat(`j`.`name`, '.',`j`.`extension`) separator ' ,') FROM tra_docupload_reqextensions t INNER JOIN par_document_extensionstypes j ON t.document_extensionstype_id = j.id WHERE t.documentupload_requirement_id = t1.id) as allowed_extensions"));
