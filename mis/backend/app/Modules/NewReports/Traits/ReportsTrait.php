...
namespace App\Modules\NewReports\Traits;
...
trait ReportsTrait
{
   ...
   public function getTotalReceivedApplications($table, $table2,$field,$filterdata,$subfilterdata,$datefilter,$has_payment_processing = null, $is_detailed_report ){
          if($table2 == ''){
            if($table=='tra_importexport_applications' || $table==='tra_importexport_applications'){
			
					$qry=DB::table($table." as t1")
					  ->leftJoin('tra_payments as tp','t1.application_code','tp.application_code')
					  ->leftJoin('tra_permitsenderreceiver_data as t1a','t1.sender_receiver_id','t1a.id')
					  ->orderBy('tp.id', 'DESC')
					  ->where($subfilterdata);
    ...
    public function  getTotalPOEInspectionsReceivedApplications($table, $table2,$field,$filterdata,$subfilterdata,$datefilter,$has_payment_processing = null, $is_detailed_report, $recommendation_id){

      $qry=DB::table($table." as t1")
      ->join('tra_poe_apprecommendations as tp','t1.application_code','tp.application_code')
      
      ->join('tra_poe_applications as tk','t1.application_code','tk.application_code')
      ->leftJoin('tra_permitsenderreceiver_data as t1a','t1.sender_receiver_id','t1a.id')
      ->orderBy('tp.id', 'DESC')
      ->where($subfilterdata)
      ->where('tp.inspection_recommendation_id',$recommendation_id);
      if($filterdata!=''){
        $qry->whereRAW($filterdata);
      }
...
public function getEvaluatedInspectedApplications($table, $table2,$field,$filterdata,$subfilterdata,$datefilter,$is_detailed_report,$module_id){
     if($table2 == ''){
          ...
          if($filterdata!=''){
            ...
            //filter by date
			if($table=='tra_premises_applications'){
				  if($datefilter!=''){
					 $datefilter = str_replace('date_filter','t2.actual_start_date',$datefilter);
					 $qry->whereRAW($datefilter);
				  }
			}
			else{
...
public function getScreenedApplications($table, $table2,$field,$filterdata, $subfilterdata,$datefilter,$is_detailed_report){
     ...
     public function funcGetQueryResponseApplications($table, $table2,$field,$filterdata, $subfilterdata,$datefilter,$is_detailed_report){
         ...
         if($table2 == ''){
            if($table=='tra_importexport_applications' || $table==='tra_importexport_applications'){
              $qry=DB::table($table.' as t1')
             ->join('checklistitems_queries as t2','t1.application_code','t2.application_code')
              ->join('checklistitems_queryresponses as t3','t2.id','t3.query_id')
              ->leftJoin('tra_permitsenderreceiver_data as t1a','t1.sender_receiver_id','t1a.id')
              ->where('t2.status',2)
             ->where($subfilterdata);
             }else{
           ...
           if($datefilter!=''){
             if($table=='tra_importexport_applications' || $table==='tra_importexport_applications'){
              $datefilter = str_replace('date_filter','t3.created_on',$datefilter);
              $qry->whereRAW($datefilter);
             }else{
