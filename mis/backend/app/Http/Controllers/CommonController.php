...
*/
	...
	public function saveGroupedApplicationPaymentDetails(Request $request)
    {
...
$results = array();
        try {
			if(validateIsNumeric($application_id)){
				 ...
				 $qry = DB::table($table_name.' as t4')
				 ->leftJoin('wb_trader_account as t1', 't1.id','t4.applicant_id')
...
public function saveChecklistApplicationQuery(Request $req)
	...
	public function generateApplicationQueryRefNo($application_code, $table_name,$query_processstage_id, $acyronym='Q'){
                $counter = 1;
                $query_ref = '';
                ...
                if($table_record){
                        ...
                        $query_ref = $reference_no.$code.'/'.$acyronym. $counter;
   ...
   public function saveUnstructuredApplicationQuery(Request $req)
    ...
    public function saveApplicationReQueryDetails(Request $request)
    {
        ...
        $checklist_querycategory_id = $request->input('checklist_querycategory_id');
        $item_resp_id = $request->input('item_resp_id');
        $user_id = $this->user_id;
        $table_name = 'checklistitems_queries';
        $where = array(
...
public function getUploadedApplicationPaymentDetails(Request $req){
        ...
        try {
            $application_code  = $req->application_code;
			//check if it has the group application code 
			$record = DB::table('tra_application_invoices')
						->select('group_application_code')
						->where('application_code',$application_code)->first();
			if($record){
				  $group_application_code  = $record->group_application_code;
				  if(validateIsNumeric($group_application_code)){
					  $application_code = $group_application_code;
           ...
           $data = array();
            ...
            $records = DB::table('tra_uploadedpayments_details as t1')
                ...
                ->leftJoin('par_currencies as t7', 't6.currency_id','t7.id')
                ->select('t1.*', 't2.*','t5.id as invoice_id', 't5.invoice_no','t6.id as receipt_id', 't6.amount_paid', 't7.name as paymentcurrency', 't3.name as currency',  't4.name as payment_mode')
...
//
				...
				if($has_invoice_generation ==1){
					...
					if($res['success']){
						...
						if($record){
							...
							$res = array('success'=>false,'hasValidatedChecklist'=>false,  'message'=>'Upload the Proof of Payment');
						}
    ...
    public function saveApplicationRejectionDetails(Request $req){
        try{
            $user_id = $this->user_id;
            $id = $req->id;
            $data = array('application_code'=>$req->application_code,'reference_section'=>$req->reference_section, 'reason_of_rejection'=>$req->reason_of_rejection);

            if(validateIsNumeric($id)){
                //update query
                $previous_data = getPreviousRecords('tra_applicationrejections_reasons', array('id' => $id));
                if ($previous_data['success'] == false) {
                    return \response()->json($previous_data);
                }
                $previous_data = $previous_data['results'];
               ...
               $data['dola'] = Carbon::now();
                ...
                $res = updateRecord('tra_applicationrejections_reasons', $previous_data, array('id' => $id), $data, $user_id);
                
            }else{
                $data['created_by'] = $user_id;
               $data['created_on'] = Carbon::now();
               
                $res = insertRecord('tra_applicationrejections_reasons', $data, $user_id);
       ...
       public function onDeleteRasonforREjections(Request $req)
       {
           ...
           try {
               ...
               $check = DB::table('tra_applicationrejections_reasons')->where($where)
                           ->first();
               if($check){
                   
                       $previous_data = getPreviousRecords('tra_applicationrejections_reasons', $where);
                       if ($previous_data['success'] == false) {
                           return $previous_data;
                       }
                       $previous_data = $previous_data['results'];
                       $res = deleteRecord('tra_applicationrejections_reasons', $previous_data, $where, $user_id);
                       if($res['success']){
                           $res = array(
                               'success' => true,
                               'message' => 'Request executed successfully!!'
                   ...
                   $res = array(
                               'success' => true,
                               'message' => 'Details not found, contact system admin!!'
                           );
       ...
       public function onDeleteAppInspectionsDeficiencies(Request $req)
    {
        ...
        try {
			...
			$check = DB::table('tra_inspectioncapa_deficiencies')->where($where)
						->first();
			if($check){
				
					$previous_data = getPreviousRecords('tra_inspectioncapa_deficiencies', $where);
					if ($previous_data['success'] == false) {
						return $previous_data;
					}
					$previous_data = $previous_data['results'];
					$res = deleteRecord('tra_inspectioncapa_deficiencies', $previous_data, $where, $user_id);
					if($res['success']){
						$res = array(
							'success' => true,
							'message' => 'Request executed successfully!!'
    ...
    public function getAllInspectionsCaparequests(Request $request)
    {
        ...
        return response()->json($res);
    }public function getAllReinspectionInspectionsRequests(Request $request)
    {
        try {
            $application_code = $request->application_code;
            $process_id = $request->process_id;
            ...
            $qry = DB::table('tra_appreinspectionrequest_reftracker as t1')
                    ->leftJoin('par_query_types as t2', 't1.query_type_id', 't2.id')
                    ->leftJoin('par_checklist_categories as t3', 't1.checklist_category_id', 't3.id')
                    ->leftJoin('users as t4', 't1.queried_by', 't4.id')
                    ->leftJoin('par_query_statuses as t5', 't1.status_id', 't5.id')
                    ->leftJoin('tra_query_invoices as t6', 't1.id', 't6.query_id')
                    ->select('t1.id as query_id', 't1.*', 't2.name as query_type', 't3.name as checklist_category', 't1.query_remark as comment','t1.status_id as status', 't5.name as query_status', 't6.invoice_id', DB::raw("CONCAT_WS(' ',decrypt(t4.first_name),decrypt(t4.last_name)) as queried_by"))
                    ->groupBy('t1.id');
        ...
        return response()->json($res);
    }public function saveReinspectionchecklistitems(Request $req)
    {
        try {
            $user_id = \Auth::user()->id;
            $post_data = $req->all();
...
public function saveApplicationCapaDeficiencies(Request $req)
    {
        try {
            $user_id = \Auth::user()->id;
            $post_data = $req->all();
    ...
    public function saveApplicationReinspectionREquests(Request $req)
    {
        try{
            $user_id = $this->user_id;
            $module_id = $req->module_id;
            ...
            if(validateIsNumeric($query_id)){
                //update query
                $previous_data = getPreviousRecords('tra_appreinspectionrequest_reftracker', array('id' => $query_id));
                if ($previous_data['success'] == false) {
                    return \response()->json($previous_data);
                }
                $previous_data = $previous_data['results'];
                ...
                $update_data = array(
                    ...
                    'reason_for_non_payment'=>$req->reason_for_non_payment,
                    'query_processstage_id'=>$query_processstage_id,
                    'comments'=>$req->comments,'queried_by' => $queried_by,
                   
                    'query_txt' => $query_txt
                );

                $res = updateRecord('tra_appreinspectionrequest_reftracker', $previous_data, array('id' => $query_id), $update_data, $user_id);
                $res['checklist_category_id'] =  $previous_data[0]['checklist_category_id'];
            }else{
                //get query type from stage
                //check for open query 
                $records = DB::table('tra_appreinspectionrequest_reftracker')
                                ->where(array('application_code'=>$application_code, 'query_processstage_id'=>$query_processstage_id, 'status_id'=>1))
                                ->count();
                if($records >0){

                    $res = array('success'=>false, 'message'=>'There is an already Existing and open Request, verify and close to proceed.');
                    return \response()->json($res);
                    exit();
                }
              
                $query_ref = $this->generateApplicationQueryRefNo($application_code, $table_name,$query_processstage_id,'/R-INSP');
                ...
                $res = insertRecord('tra_appreinspectionrequest_reftracker', $query_data, $user_id);
 ...
 public function getApplicationRejectiondetails(Request $request){
        try {
            ...
            $qry = DB::table('tra_applicationrejections_reasons as t1')
                ->select('t1.*', 't1.id')
                ->where($where)
                ->orderBy('t1.id', 'DESC');
            $results = $qry->first();
    ...
    }   public function getreinspectionsrequestsitems(Request $request)
    {
        try{
            $query_id = $request->input('query_id');
            
            $record = DB::table('reinspectiontitems_queries as t1')
                       ->select('t1.*');
                        if(validateIsNumeric($query_id)){
                            $record->where('query_id',$query_id);
                        }
