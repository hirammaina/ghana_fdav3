...
if(!validateIsNumeric($req->product_origin_id)){
	...
	function getProductInformationRequests($req){
		$data = array('common_name_id'=>$req->common_name_id,
...
public function onSearchRegisteredProductApplication(Request $req){
    try {
       ...
       ->select('t7.*','t1.*', 't16.name as section_name', 't4.name as validity_status','t15.name as registration_status', 't1.id as active_application_id', 't1.reg_product_id', 't3.name as applicant_name', 't9.name as local_agent', 't12.id as registered_product_id',
           't13.name as storage_condition','t7.brand_name', 't8.name as common_name', 't10.name as classification_name', 't11.certificate_no', 
           't7.brand_name as sample_name','t14.manufacturer_id', DB::raw("if(t12.tra_product_id >0,t12.tra_product_id,t1.product_id) as  tra_product_id"))
 ...
 public function onSaveRenetionRequestApplication(Request $req){
        try { 
            $product_res =  '';
                ...
                $retentionapp_data = array('retention_yearfrom'=>$retention_yearfrom,'retention_yearto'=>$retention_yearto,
...
public function getAnnualretentionRegisteredProducts(Request $req){
    ...
    public function saveProductRetentionSelection(Request $req){
        try{
            ...
            $trader_id = $req->trader_id;
            $email_address = $req->email_address;
            $retentionapp_data = array('application_code'=>$application_code,
...
public function onSearchFoodProductProductApplication(Request $req){
    try {
       ...
       ->select('t7.*','t1.*', 't1.application_code as reference_application_code','t16.name as section_name', 't4.name as validity_status','t15.name as registration_status', 't1.id as active_application_id', 't1.reg_product_id', 't3.name as applicant_name', 't9.name as local_agent', 't12.id as registered_product_id','t1.product_id as tra_product_id',
           't13.name as storage_condition','t7.brand_name', 't12.tra_product_id', 't8.name as common_name', 't10.name as classification_name', 't11.certificate_no', 
           't7.brand_name as sample_name','t14.manufacturer_id',DB::raw("5 as prodclass_category_id"))
		   ->whereNull('t11.id')
