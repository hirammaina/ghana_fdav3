...
class OpenOfficeController extends Controller
{
    ...
    {
        \App::call('App\Modules\AuditReport\Http\Controllers\AuditReportController@logPromotionAndAdvertisementApplication', [4]);
    }
    public function getProductsApplicationColumns(request $request)
    {
        try {
            ...
            $qry = DB::table('tra_product_applications as t1')
                ...
                ->LeftJoin('tra_premises as t14', 't1.local_agent_id', 't14.id')
                ...
                foreach ($filters as $key => $value) {
                    ...
                    if ($key == 't1.sub_module_id' && validateIsNumeric($value)) {
                        if ($value == 103) {
                            ...
                            $qry->select(DB::raw("DISTINCT t40.id as register_id,t1.application_code,t1.id as applicant_id,t1.product_id,t1.reference_no,t1.tracking_no,t1.submission_date,t1.submission_date as ReceivedFrom,t1.submission_date as ReceivedTo,t1.section_id,t2.brand_name, t2.warnings,t2.shelf_life,t2.shelf_lifeafter_opening,t2.instructions_of_use,t2.physical_description, t3.name as Classification, t4.name as commonName,t5.name as Category,t6.name as SubCategory,t7.name as SpecialCategory,t8.name as StorageCondition,t9.name as ProductForm,t10.name as IntendedUsers,t2.shelflifeduration_desc,t11.name as issueplace,t12.name as ProductType,t13.name as Applicant,t13.postal_address as ApplicantPostalA,t13.physical_address as ApplicantPhysicalA,t13.email_address as ApplicantEmail,t13.telephone_no as ApplicantTell,t13.mobile_no as ApplicantMobile,t14.name as LocalAgent,t14.postal_address as LocalAgentPostalA,t14.physical_address as LocalAgentPhysicalA,t14.email_address as 
    ...
    public function getPremiseApplicationColumns(request $request)
    {
        try {
            $filters = $request->input('filters');
            $business_category = $request->input('BsnCategory');
            $business_type = $request->input('BsnType');
            $category = $request->input('Category');
            $business_scale = $request->input('BsnScale');
            $decision_id = $request->input('decision');
            ...
            $qry = DB::table('tra_premises_applications as t1')
                ...
                ->select('t1.id as application_id', 't1.applicant_id', 't1.application_code', 't1b.id as premise_id', 't1.reference_no', 't1.tracking_no', 't1b.name', 't1b.email', 't1b.postal_address', 't1b.physical_address', 't1b.telephone', 't1b.mobile_no', 't1b.contact_person_startdate', 't1b.contact_person_enddate', 't1b.gps_coordinate', 't2.name as premise_country', 't3.name as premise_region', 't4.name as premise_district', 't5.name as business_type', 't7.name as business_category', 't6.name as business_scale', 't8.name as trader', 't8.postal_address as trader_postal_address', 't8.physical_address as trader_physical_address', 't8.email as trader_email', 't8.telephone_no as trader_telephone', 't8.mobile_no as trader_mobile_no', 't9.name as contact_person', 't9.telephone_no as contact_telephone_no', 't9.email_address as contact_email_address', 't11.name as business_type_details', 't12.name as place_of_issue', 't13.name as trader_country', 't14.name as trader_region', 't15.expiry_date as certificate_expiry_date', 't15.certificate_issue_date as certificate_issue_date', 't16.name as premise_category', 't15.certificate_issue_date as issue_from', 't15.certificate_issue_date as issue_to', 't1.date_added as received_from', 't1.date_added as received_to', 't15.permit_no as certificate_no', 't23.name as registration_status', 't24.name as validity_status', DB::raw("if(t1.reference_no != '', reference_no, tracking_no) as reference_no"));
            ...
            if (isset($filters)) {
                ...
                foreach ($filters as $key => $value) {
                    ...
                    if ($key == 't1.premise_type_id' && validateIsNumeric($value)) {
                        $qry->where('t1b.premise_type_id', $value);
                    }
                    //dates
                    if (isset($filters['to_date']) != null && $filters['from_date'] != null) {
                        $to_date = $filters['to_date'];
            ...
            if (validateIsNumeric($business_category)) {
                $qry->where('t1b.business_category_id', $business_category);
            }
            if (validateIsNumeric($business_type)) {
                $qry->where('t1b.business_type_id', $business_type);
            }
            if (validateIsNumeric($premise_type_id)) {
                $qry->where('t1b.premise_type_id', $premise_type_id);
            }
            if (validateIsNumeric($category)) {
                $qry->where('t1b.premise_type_id', $category);
            }
            if (validateIsNumeric($business_scale)) {
                $qry->where('t1b.business_scale_id', $business_scale);
            }
            // if(validateIsNumeric($place_of_issue)){
            //       $qry->where('t1.zone_id' , $place_of_issue);
            //      }
            if (validateIsNumeric($validity_status)) {
                    ...
                    foreach ($filters as $filter) {
                        switch ($filter->property) {
                            ...
                            case 'tracking_no':
                                ...
                                break;
                            case 'reference_no':
                                $whereClauses[] = "(t1.reference_no like '%" . ($filter->value) . "%' or t1.tracking_no like '%" . ($filter->value) . "%')";
                                break;
                            case 'email':
                            ...
                            case 'gps_coordinate':
                                ...
                                break;
                            case 'premise_country':
                                $whereClauses[] = "t2.name like '%" . ($filter->value) . "%'";
                                break;
                            case 'premise_region':
                                $whereClauses[] = "t3.name like '%" . ($filter->value) . "%'";
                                break;
                            case 'premise_district':
                                $whereClauses[] = "t4.name like '%" . ($filter->value) . "%'";
                                break;
                            case 'business_type':
                                $whereClauses[] = "t5.name like '%" . ($filter->value) . "%'";
    ...
    public function getIESpreadSheet(request $request)
    {
        try {
            $qry = DB::table('tra_importexport_applications as t1')
                ...
                ->LeftJoin('par_modesof_transport as t30', 't1.mode_oftransport_id', 't30.id')

                ->LeftJoin('par_permitsproduct_categories as t31', 't1.permit_productscategory_id', 't31.id')
                ->LeftJoin('tra_payments as t34', 't1.application_code', 't34.application_code')
                ->LeftJoin('par_currencies as t35', 't34.currency_id', 't35.id')
                ->LeftJoin('tra_application_invoices as t36', 't1.application_code', 't36.application_code')
                ->LeftJoin('tra_invoice_details as t37', 't36.id', 't37.invoice_id')
                ->LeftJoin('par_currencies as t38', 't37.paying_currency_id', 't38.id')

                ->select(DB::raw("t36.invoice_no, t37.total_element_amount as invoice_amount,t38.name as invoice_currency, t35.name as pay_currency,t34.receipt_no as receipt_no,t34.amount_paid,  t30.name as mode_oftransport,t31.name as permit_productscategory, t25.name as section_name, t1.proforma_invoice_no,t1.reference_no,t1.tracking_no,t1.application_code,t1.proforma_invoice_date,t3.name as permitreason,t2.name as type,t3.name as category,t4.name as typecategory,t6.name as port,t7.name as currency,t8.name as consigneeoption,t9.name as consignee,t9.postal_address as Cpostal_address,t9.physical_address as Cphysical_address,t9.telephone_no as Ctelephone_no,t9.mobile_no as Cmobile_no,t9.email_address as Cemail_address,t15.name as Ccountry,t16.name as Cregion,t10.name as senderreceiver,t10.physical_address as SRphysical_address,t10.postal_address as SRpostal_address,t10.telephone_no as SRtelephone_no,t10.mobile_no as SRmobile_no,t10.email as SRemail_address,t13.name as SRcountry,t14.name as SRregion,t11.name as premisename,t11.postal_address as premisePostalA,t11.physical_address as premisePhysicalA,t11.telephone as premiseTell,t11.mobile_no as premiseMobile,t11.expiry_date as premiseExpiryDate,t12.name as issueplace,t17.expiry_date as CertExpiryDate,t17.certificate_issue_date as CertIssueDate,t18.name as Applicant,t18.postal_address as ApplicantPostalA,t18.physical_address as ApplicantPhysicalA,t18.telephone_no as ApplicantTell,t18.mobile_no as ApplicantMobile,t18.email as ApplicantEmail,t19.name as ApplicantCountry,t20.name as ApplicantRegion,t17.certificate_issue_date as IssueFrom,t17.certificate_issue_date as IssueTo,t1.date_added as ReceivedFrom,t1.date_added as ReceivedTo,t17.certificate_no, t23.name as registration_status, t24.name as validity_status"));

            // $qry->groupBy('t1.application_code');
            //if part par_permitsproduct_categories par_sections regulated_producttype_id
            if (!isset($request->IDs)) {
                ...
                $regulated_producttype_id = $request->regulated_producttype_id;
                ...
                if (isset($filters)) {
                    ...
                    foreach ($filters as $key => $value) {
                        ...
                        if ($key == 't1.regulated_producttype_id' && validateIsNumeric($value)) {
                            $qry->where('t25.regulated_producttype_id', $value);
                        }
                        if ($key == 't1.sub_module_id' && validateIsNumeric($value)) {
                            $qry->where('t1.sub_module_id', $value);
                        }
                ...
                if (validateIsNumeric($regulated_producttype_id)) {
                    $qry->where('t25.regulated_producttype_id', $regulated_producttype_id);
                }
                ...
                if (validateIsNumeric($permit_reason)) {
                    $qry->where('t1.permit_category_id', $permit_reason);
                }
                ...
                $received_to_date = ($request->input('received_to_date'));
                $received_from_date = ($request->input('received_from_date'));

                if ($approved_from_date != '') {
                    $qry->whereRaw("date_format(t17.certificate_issue_date, '%Y%-%m-%d') >= '" . formatDate($approved_from_date) . "'");
                }

                if ($approved_to_date != '') {
                    $qry->whereRaw("date_format(t17.certificate_issue_date, '%Y%-%m-%d') <= '" . formatDate($approved_to_date) . "'");
                }
                if ($received_to_date != '') {
                    $qry->whereRaw("date_format(t1.date_added, '%Y%-%m-%d') <= '" . formatDate($received_to_date) . "'");
                }
                if ($received_from_date != '') {
                    $qry->whereRaw("date_format(t1.date_added, '%Y%-%m-%d') >= '" . formatDate($received_from_date) . "'");
                }
                $filter = $request->input('filter');
                $whereClauses = array();
                $filter_string = '';
                ...
                if (isset($start) && isset($limit)) {
                    $results = $qry->groupBy('t1.application_code')->skip($start)->take($limit)->get();
                } else {
                    $results = $qry->groupBy('t1.application_code')->get();
                }
    ...
    public function getIEPermitSpreadSheet(request $request)
    {
        try {
            ...
            $qry = DB::table('tra_permits_products as t1a')
                ...
                ->leftJoin('par_countries as t19', 't18.country_id', 't19.id')
                ->leftJoin('par_regions as t20', 't18.region_id', 't20.id')
                ->LeftJoin('par_permits_reviewrecommendations as t21', 't17.decision_id', 't21.id')
                ->LeftJoin('par_common_names as t22', 't2a.common_name_id', 't22.id')
                ->LeftJoin('tra_manufacturers_information as t23', 't1a.manufacturer_id', 't23.id')
                ->LeftJoin('tra_product_manufacturers as t24', 't2a.id', 't24.product_id')
                ->LeftJoin('par_countries as t26', 't23.country_id', 't26.id')
                ->LeftJoin('tra_product_manufacturers as t27', 't2a.id', 't27.product_id')
                ->LeftJoin('tra_manufacturers_information as t28', 't27.manufacturer_id', 't28.id')
                ->LeftJoin('par_countries as t29', 't28.country_id', 't29.id')
                ->LeftJoin('par_modesof_transport as t30', 't1.mode_oftransport_id', 't30.id')

                ->LeftJoin('par_permitsproduct_categories as t31', 't1.permit_productscategory_id', 't31.id')
                ->LeftJoin('par_sections as t32', 't1.section_id', 't32.id')
                ->LeftJoin('par_dosage_forms as t33', 't2a.dosage_form_id', 't33.id')
                ->LeftJoin('tra_payments as t34', 't1.application_code', 't34.application_code')
                ->LeftJoin('par_currencies as t35', 't34.currency_id', 't35.id')
                ->LeftJoin('tra_application_invoices as t36', 't1.application_code', 't36.application_code')
                ->LeftJoin('tra_invoice_details as t37', 't36.id', 't37.invoice_id')
                ->LeftJoin('par_currencies as t38', 't37.paying_currency_id', 't38.id')
                ->LeftJoin('par_sections as t39', 't1.section_id', 't39.id')
                ->LeftJoin('tra_product_applications as t40', 't1a.product_id', 't40.product_id')
                ->LeftJoin('tra_approval_recommendations as t41', 't40.application_code', 't41.application_code')
                //  ->LeftJoin('par_registration_statuses as t23','t17.appregistration_status_id','t23.id') section_name mode_oftransport
                //  ->LeftJoin('par_validity_statuses as t24','t17.appvalidity_status_id','t24.id') senderreceiver par_permit_category mode_oftransport issueplace ReceivedFrom CertIssueDate t26 permit_productscategory_id tra_product_applications if(t40.sub_module_id == 30 )

                ->select(DB::raw("CASE when t41.id > 0 && t40.sub_module_id <> 30 then 'Registered Product' when t40.sub_module_id = 30 then 'Authorised Product' else  'Not Registered/Authorised' END as productapproval_type_id,t1a.permitprod_recommendation_id,t1a.permitprod_recommendation_remarks,  t36.invoice_no,if(t17.decision_id <1,'',t21.name) as review_recommendation, t37.total_element_amount as invoice_amount,t38.name as invoice_currency, t35.name as pay_currency,t34.receipt_no as receipt_no,t34.amount_paid, t39.name as section_name, t33.name as dosage_form, t1.reference_no,t31.name as permit_productscategory, t1.tracking_no, if(t1a.product_id >0, t2a.brand_name,t1a.permitbrand_name) as brand_name,if(t22.id >0, t22.name , t1a.permitcommon_name) as common_name,t2a.product_strength,t2a.physical_description as productphysical_description,if(t1a.manufacturer_id >0,t23.name, t28.name) as manufacturer_name, if(t26.id >0, t26.name, t29.name ) as country_oforigin,t1a.product_batch_no,t1a.product_manufacturing_date,t1a.product_expiry_date, t1a.quantity, t3a.name as packaging_unit,t1a.total_weight as weight, t4a.name as weight_unit,t1a.unit_price, (t1a.quantity*t1a.unit_price) as total,t5a.name as currency, t18.name as Applicant,t18.postal_address as ApplicantPostalA,t18.physical_address as ApplicantPhysicalA,t18.telephone_no as ApplicantTell,t18.mobile_no as ApplicantMobile,t18.email as ApplicantEmail,t19.name as ApplicantCountry,t20.name as ApplicantRegion,t11.name as premisename,t11.postal_address as premisePostalA,t11.physical_address as premisePhysicalA,t11.telephone as premiseTell,t11.telephone as premiseMobile,t11.expiry_date as premiseExpiryDate,t9.name as consignee,t9.postal_address as Cpostal_address,t9.physical_address as Cphysical_address,t9.telephone_no as Ctelephone_no,t9.mobile_no as Cmobile_no,t9.email_address as Cemail_address,t15.name as Ccountry,t16.name as Cregion, t10.name as senderreceiver,t10.physical_address as SRphysical_address,t10.postal_address as SRpostal_address,t10.telephone_no as SRtelephone_no,t10.mobile_no as SRmobile_no,t10.email as SRemail_address,t13.name as SRcountry,t14.name as SRregion,t3.name as category, t3.name as permitreason,t4.name as typecategory,t6.name as port,t30.name as mode_oftransport,t12.name as issueplace, t1.proforma_invoice_no,t1.proforma_invoice_date,t1.date_added as ReceivedFrom,t1.date_added as ReceivedTo,t17.expiry_date as CertExpiryDate,t17.certificate_issue_date as CertIssueDate,t17.certificate_issue_date as IssueTo,t17.permit_no as certificate_no,t17.appregistration_status_id as registration_status,t17.appregistration_status_id as validity_status, t17.expiry_date as CertExpiryFromDate,t17.expiry_date as CertExpiryToDate"));

            // $qry->groupBy('t1a.id');
            //	dd($qry->toSql()); //ApplicantEmail par_permit_category
            $filters = $request->input('filters');
            $permit_category = $request->input('permit_category');
            $import_typecategory = $request->input('import_typecategory');
            $permit_reason = $request->input('permit_reason');
            ...
            $received_to_date = $request->input('received_to_date');
            $received_from_date = $request->input('received_from_date');
            //filers for combo premiseMobile
            $filters = (array)json_decode($filters);
            ...
            if (isset($filters)) {
                ...
                foreach ($filters as $key => $value) {
                    ...
                    if ($key == 't1.regulated_producttype_id' && validateIsNumeric($value)) {
                        $qry->where('t39.regulated_producttype_id', $value);
                    }
                    ...
                    foreach ($filters as $filter) {
                        ...
                        switch ($filter->property) {
                            case 'brand_name':
                                ...
                                break;
                            case 'product_strength':
                                $whereClauses[] = "t2a.product_strength like '%" . ($filter->value) . "%'";
                                break;
                            case 'productphysical_description':
                                $whereClauses[] = "t2a.physical_description like '%" . ($filter->value) . "%'";
                                break;
                            case 'invoice_no':
                                $whereClauses[] = "t36.invoice_no like '%" . ($filter->value) . "%'";
                                break;

                            case 'receipt_no':
                                $whereClauses[] = "t34.receipt_no like '%" . ($filter->value) . "%'";
                                break;
                            case 'manufacturer_name':
                                $whereClauses[] = "( t23.name like '%" . ($filter->value) . "%' or  t28.name like '%" . ($filter->value) . "%') ";
                                break;

                            case 'common_name':
                                $whereClauses[] = "( t22.name like '%" . ($filter->value) . "%' or t1a.permitcommon_name like '%" . ($filter->value) . "%') ";
                                break;
                            case 'product_batch_no':
                                $whereClauses[] = "t1a.product_batch_no like '%" . ($filter->value) . "%'";
                                break;
                                //product_manufacturing_date,t1a.product_expiry_date
                            case 'product_manufacturing_date':
                                $whereClauses[] = "date_format(t1a.product_manufacturing_date, '%Y%-%m-%d')= '" . formatDate($filter->value) . "'";
                                break;
                            case 'product_expiry_date':
                                $whereClauses[] = "date_format(t1a.product_expiry_date, '%Y%-%m-%d')= '" . formatDate($filter->value) . "'";
                                break;
                            case 'quantity':
                            ...
                            case 'reference_no':
                                $whereClauses[] = "t1.reference_no like '%" . ($filter->value) . "%'";
                                break;
                            case 'mode_oftransport':
                                $whereClauses[] = "t1.mode_oftransport_id = '" . ($filter->value) . "'";
                                break;
    ...
    public function exportall(request $request)
    {
        ...
        if (empty($data)) {
            $response =  array(
                'success' => false,
                'message' => 'Sorry!!! No data to Export'
            );
        } else {
    ...
    public function getUploadedDocumentPerApplication(request $request)
    {
        ...
        if (isset($application_code)) {
            $qry = DB::table('tra_application_uploadeddocuments as t2')
                ...
                ->where('t2.application_code', $application_code)
                ->select('t2.file_name', 't2.*', 't2.initial_file_name', 't2.file_type', 't2.remarks', 't2.uploaded_on', 't5.name as document_type', 't4.name as uploaded_by');
        ...
        try {
            $qry = DB::table('tra_surveillance_applications as t1')
                ...
                ->select(
                    ...
                    't1.program_implementation_id',
                    'tssd.sample_refno',
                    'tssd.sample_name',
                    'tssd.sample_code',
                    'tdc.name as dosage_form',
                    'tpf.name as product_form',
                    'tdt.name as device_type',
                    'tssd.date_collected',
                    't9.name as classification',
                    'tssd.packaging_size',
                    't10.name as packaging_unit',
                    'tssd.collected_samples',
                    'tssd.batch_no',
                    'tmi.name as manufacturer',
                    'tssd.manufacturing_date',
                    'tssd.expiry_date',
                    't11.name as storage',
                    'tssd.collectionsite_storage_condition',
                    't12.name as seal_condition',
                    'tssd.shelf_life',
                    'tssd.shelf_lifeafter_opening',
                    'tsreason.name as sampling_reason',
                    DB::raw("'' as Classification, '' as common_name, CONCAT_WS(' ', decrypt(users.first_name), decrypt(users.last_name) ) as collector"),
                    't13.name as sample_type',
                    'tpd.name as program_name',
                    'tpd.start_date as program_start_date',
                    'tpd.end_date as program_end_date',
                    'tpd.description as program_description',
                    'tcn.name as product_name',
                    'tsr.name as pms_region_name',
                    'tsd.name as pms_district',
                    'tbt.name as site_name',
                    'tssb.name as sampling_site_category',
                    'tss.email as site_email',
                    'tsdc.name as site_country',
                    'tsr.name as site_region_name',
                    'tsd.name as site_district',
                    'tss.email as email_address',
                    'tss.postal_address as postal_address',
                    'tss.physical_address as physical_address',
                    'tss.telephone as telephone_no',
                    'tss.fax',
                    'tss.website',
                    'tss.street',
                    'tpi.program_implementation as implementation_identity',
                    'tpi.year_of_implementation as implementation_year',
                    'tpi.implementationstart_date as implementation_start_date',
                    'tpi.implementationend_date as implementation_end_date',
                    'tta.name as Trader',
                    'tta.postal_address as TraderPostalA',
                    'tta.physical_address as TraderPhysicalA',
                    'tta.telephone_no as TraderTell',
                    'tta.mobile_no as TraderMobile',
                    'tta.email as TraderEmail',
                    'ttac.name as TraderCountry',
                    'ttar.name as TraderRegion',
                    't15.name as site_level',
                    't19.expiry_date as CertExpiryDate',
                    't19.certificate_issue_date as CertIssueDate',
                    't16.name as issueplace',
                    't19.certificate_issue_date as IssueFrom',
                    't19.certificate_issue_date as IssueTo',
                    't1.date_received as ReceivedFrom',
                    't1.date_received as ReceivedTo',
                    't20.name as registration_status',
                    't22.name as pir_recommendation',
                    't21.comments as pir_comment',
                    't21.created_on as pir_date',
                    't23.name as screening_recommendation',
                    't24.name as conformatory_recommendation',
                    't25.limsreference_no',
                    't25.requested_on',
                    DB::raw("CONCAT_WS(' ', decrypt(t27.first_name), decrypt(t27.last_name)) as request_by"),
                    't28.name as analysis_type',
                    't26.name as sample_analysis_status'
                )
                ->groupBy('t1.application_code');
            $filters = $request->input('filters');
    ...
    public function getApprovalDetails(Request $req)
    {
        $application_code = $req->input('application_code');
        $table_name = 'tra_approval_recommendations';

        try {
            $qry = DB::table($table_name . ' as t2', 't1.application_code', '=', 't2.application_code')
                ->LeftJoin('users as t3', 't2.approved_by', 't3.id')
                ->leftJoin('par_titles as t4', 't3.title_id', '=', 't4.id')

                ->select(DB::raw("DISTINCT t2.id,t2.*,CONCAT_WS(' ',t4.name,decrypt(t3.first_name),decrypt(t3.last_name)) as approved_by_name"));

            $qry->where('t2.application_code', $application_code)->groupBy('t2.application_code');
