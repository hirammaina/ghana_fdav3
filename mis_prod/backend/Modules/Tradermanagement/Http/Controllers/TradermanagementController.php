...
//REPLACE(`col_name`, ' ', '')
   ...
   public function DeleteTraderNotificationMail(Request $req){
	  ...
	  public function getApplicationUploadProofAuthorisation(Request $req){
			try{
				 $records = DB::table('tra_applicationuploadproof_authorisation as t1')
								->leftJoin('users as t2', 't1.authorised_by_id', 't2.id')
								->leftJoin('par_payuploadproofauth_types as t3', 't1.payuploadproofauth_type_id', 't3.id')
								->leftJoin('modules as t4', 't1.module_id', 't4.id')
								->leftJoin('par_authorisation_statuses as t5', 't1.authorisation_status_id', 't5.id')
								->leftJoin('wb_trader_account as t6', 't1.applicant_id', 't6.id')
								->select(DB::raw("t1.*, t6.name, t3.name as payuploadproofauth_type,t6.identification_no, t5.name as authorisation_status, CONCAT_WS(' ',decrypt(t2.first_name),decrypt(t2.last_name)) as authorised_by, t4.name as module_name, t1.created_on as effected_on, CONCAT_WS(' ',decrypt(t2.first_name),decrypt(t2.last_name)) as effected_by"))
								->get();
				 $res = array(
						'success' => true,
						'results' => $records,
